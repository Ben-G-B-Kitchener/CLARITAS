#!/usr/bin/env python3
"""CLARITAS V24.36 Mie-two-moment-constrained finite-interface particle kernel.
...
__device__ int spheroid_fresnel_scatter_direction_only_3d(
    ...
    *vx=tx;*vy=ty;*vz=tz;
    ...
    for(int bounce=0;bounce<=max_bounces;++bounce){
        ...
        if(alpha_abs>0.0f){
            float survival=expf(-alpha_abs*chord);
            if(rnd_uniform(state)>survival){*internal_path+=chord;(*p_absorptions)++;return 2;}
        }
        x+=chord*(*vx);y+=chord*(*vy);z+=chord*(*vz);*internal_path+=chord;
            ...
            if(use_wave){
                ...
                if(coherent){
                    ...
                    int ps=spheroid_fresnel_scatter_direction_only_3d(&stateOPT,N_WATER,(float)particle_n[pidx],(float)particle_k[pidx],PARTICLE_WAVELENGTH,PARTICLE_SURFACE_ROUGH_ALPHA,
                        ...
                        MAX_PARTICLE_BOUNCES,&vx,&vy,&vz,&event_internal_path,&pr,&per,&pir,&pabs);
                    if(ps==2){st=5;break;}
                    if(ps!=1){st=4;break;}
                ...
                int ps=sphere_fresnel_interaction_3d(&stateOPT,N_WATER,(float)particle_n[pidx],(float)particle_k[pidx],PARTICLE_WAVELENGTH,PARTICLE_SURFACE_ROUGH_ALPHA,(float)particle_radius[pidx],
                    MAX_PARTICLE_BOUNCES,&x,&y,&z,&vx,&vy,&vz,&event_internal_path,&pr,&per,&pir,&pabs);
                if(ps==2){st=5;break;}
                if(ps!=1){st=4;break;}
                event_type=(per>per_before)?1:((pir>pir_before)?3:2);
            }
...
class TardiisForwardModel:
    ...
    def simulate(self,material,concentration_g_per_L,weights=None,n_rays=100000,seed=12345,
        ...
        total_rays=0;valid_air_total=0;entered_total=0;source_accept_total=0;source_direct_accept_total=0;source_reflected_accept_total=0;source_bore_interactions_total=0;source_bore_reflections_total=0;axial_total=0;top_exit_total=0;bottom_exit_total=0
        ...
        while total_rays<maxN:
            ...
            for local_start in range(0,batchN,self.chunk_size):
                ...
                outf=[cp.full(n,cp.nan,dtype=cp.float32) for _ in range(13)];outi=[cp.zeros(n,dtype=cp.int32) for _ in range(26)]
