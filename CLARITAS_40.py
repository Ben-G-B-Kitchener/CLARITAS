...
# CLARITAS 4.0 optical-object model.
#
...
# Floc absorption is treated as effective absorption through a porous aggregate.
# The absorbing fraction scales with solid volume fraction inferred from the
# fractal effective density. This avoids making every floc behave like a solid
# compact absorbing sphere.
...
#
# Flocs are NOT treated as Mie spheres in CLARITAS 4.0.
...
#   k_eff = PRIMARY_REFRACTIVE_INDEX_IMAG_K * solid_volume_fraction
...
for idx, (r, refr_index, is_floc) in enumerate(zip(
    ...
    if is_floc:
        ...
        solid_volume_fraction = (
            particle_density_by_bin_kg_per_m3[idx] /
            particle_density_kg_per_m3
            if particle_density_kg_per_m3 > 0.0 else 0.0
        )
        solid_volume_fraction = np.clip(solid_volume_fraction, 0.0, 1.0)
        ...
        internal_path_m = FLOC_ABSORPTION_PATH_FACTOR * particle_diameter_m[idx]
        tau_abs = (
            4.0 * np.pi * k_eff * internal_path_m /
        ...
        sigma_a_this = area * (1.0 - np.exp(-tau_abs))
        ...
        m_rel = (n_particle - 1j * PRIMARY_REFRACTIVE_INDEX_IMAG_K) / n_medium
...
STEP_SIZE = STEP_LENGTH_FACTOR / mu_t if mu_t > 0.0 else np.inf

scatter_prob_per_step = (
    1.0 - np.exp(-mu_t * STEP_SIZE)
    if mu_t > 0.0 and np.isfinite(STEP_SIZE)
    else 0.0
)
# Diagnostic reference values only.
STEP_SIZE_MU_S_0P1_REFERENCE = 0.1 / mu_t if mu_t > 0.0 else np.inf
STEP_SIZE_MU_S_1P0_REFERENCE = 1.0 / mu_t if mu_t > 0.0 else np.inf
STEP_SIZE_MU_S_PRIME_REFERENCE = (
    1.0 / mu_s_prime if mu_s_prime > 0.0 else np.inf
)

# Particle/floc choice during an extinction event is weighted by extinction
# contribution, not by raw PSD mass fraction.
particle_event_weights = np.zeros_like(mu_t_by_bin, dtype=np.float64)

if mu_t > 0.0:
    particle_event_weights = mu_t_by_bin / mu_t
...
# CLARITAS 4.0 angular-profile cache.
#
# Primary bins:
...
mie_cache_version = "claritas_3p2_primary_mie_floc_synthetic_pair_structure_transport_v2_memorysafe"
    ...
    unsigned int seed2)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            float omega_this = (float)single_scattering_albedo_table[pidx];
            omega_this = fminf(1.0f, fmaxf(0.0f, omega_this));

            // Generic albedo rule: an extinction event either scatters or absorbs.
            // If absorbed, terminate the ray. This applies to both primary and floc bins.
            if (rnd_uniform(&state) > omega_this) {
                absorbed = 1;
                break;
            }
            ...
            // Floc bins contain synthetic fractal aggregate structure-factor CDFs.
