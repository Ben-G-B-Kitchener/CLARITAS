...
scatter_prob_per_step = None

# ============================ FLOCCULATION MODEL ============================
# CLARITAS 3.0 floc model.
#
...
#   - floc angular scattering is sampled from an RDG-FA style
#     aggregate structure-factor CDF: primary-material form factor x S(q)
#
# No floc Mie sphere calculation.
...
# Flocs use a host-built RDG-FA style aggregate angular CDF.
# The kernel samples the per-bin angular CDF for both primaries and flocs.
...
# ============================ PRIMARY-PARTICLE REFRACTIVE INDEX ============================
# Primary particles use Mie scattering and therefore require a refractive index.
#
# Flocs are NOT treated as Mie spheres in CLARITAS 3.0.
# They do not use a refractive index for cross-section or angular scattering.
# Their effective density is retained only as a diagnostic from the fractal mass model.
...
# Diagnostic-only floc refractive index placeholder.
# This is deliberately NaN for flocs to prevent accidental use in floc Mie physics.
if np.any(particle_is_floc):
    particle_refractive_index_by_bin[particle_is_floc] = np.nan
...
# Scattering cross-section and anisotropy per effective bin.
#
# Primary particles:
#   sigma_s = Qsca * pi*r^2    from Mie
#   g       = Mie asymmetry parameter
#
# Flocs:
#   sigma_s = FLOC_SCATTER_EFFICIENCY * pi*r^2
#   g       = 0 diagnostic placeholder; the actual floc angular law is
#             sampled from the RDG-FA CDF generated below.
#
# Therefore floc event rate is no longer Mie-derived.
sigma_s = []
g_by_bin = []

for r, refr_index, is_floc in zip(
    particle_radius_m,
    particle_refractive_index_by_bin,
    particle_is_floc
):
    if is_floc:
        sigma_s.append(FLOC_SCATTER_EFFICIENCY * np.pi * r**2)
        g_by_bin.append(0.0)
    else:
        m_rel = refr_index / n_medium
...
# Floc g values are not represented by a single RDG-FA g in this RDG-FA variant,
# so floc g_by_bin is retained as a neutral diagnostic placeholder.
...
scatter_prob_per_step = (
    1.0 - np.exp(-mu_s * STEP_SIZE)
    if mu_s > 0.0 and np.isfinite(STEP_SIZE)
    else 0.0
)
...
STEP_SIZE_MU_S_PRIME_REFERENCE = (
    1.0 / mu_s_prime if mu_s_prime > 0.0 else np.inf
)

# Particle/floc choice during a scattering event should be weighted by
# contribution to scattering interactions, not by raw PSD mass fraction.
...
# ================= ANGULAR PROFILES (host side - primary Mie + floc RDG-FA, cached) =================
theta_rad = np.deg2rad(theta_deg)

# CLARITAS 3.0 angular-profile cache.
#
# Primary bins:
...
#
# Floc bins:
#   - RDG-FA style angular intensity: representative primary-material
#     Mie form factor multiplied by a fractal aggregate structure factor S(q)
#   - no floc Mie sphere calculation
#   - no floc refractive index
#
# The CUDA kernel samples angle_cdf_table for both primary and floc bins.

mie_cache_version = "claritas_3p1_primary_mie_floc_rdgfa_v1"
...
def rdgfa_structure_factor(q_values, rg_m, fractal_dimension, monomer_count):
    """
    Lightweight RDG-FA style mass-fractal structure factor.

    This is a deliberately compact structure-factor approximation suitable for
    Monte-Carlo angular sampling. It uses only quantities already present in the
    CLARITAS floc model: aggregate size/mass and fractal dimension.

    S(q) is normalised to unity at q=0 for angular-shape sampling, because the
    absolute event rate is already handled by sigma_floc and mu_s_by_bin.
    """
    qrg = np.maximum(q_values * max(rg_m, 1.0e-30), 0.0)
    df = np.clip(fractal_dimension, 1.0, 3.0)

    # Smooth fractal roll-off: S_norm(0)=1 and S_norm ~ (q Rg)^(-Df)
    # at high q. This captures the loss of phase coherence as scattering angle
    # increases without introducing forward/side/back empirical fractions.
    s_norm = (1.0 + (qrg * qrg)) ** (-0.5 * df)

    # Prevent exact zeros in the far tail so CDF construction remains robust.
    return np.maximum(s_norm, 1.0e-300).astype(np.float64)


def representative_primary_mie_intensity(theta_rad_values, wl, source_mid_diameter_m):
    """Mie form factor for representative primary material within a floc band."""
    representative_radius = max(source_mid_diameter_m * 0.5, 1.0e-30)
    mu_values = np.cos(theta_rad_values)
    m_rel = n_particle / n_medium
    x = 2.0 * np.pi * n_medium * representative_radius / wl
    S1, S2 = miepython.S1_S2(m_rel, x, mu_values)
    intensity = 0.5 * (np.abs(S1)**2 + np.abs(S2)**2)
    intensity = np.real(intensity).astype(np.float64)
    return np.maximum(intensity, 1.0e-300)


def rdgfa_floc_intensity_profile(theta_rad_values, wl, bin_idx):
    """
    RDG-FA style floc angular profile for one effective floc bin.

    I(theta) = P_primary(theta) * S(q)

    P_primary is represented by the Mie intensity of the source-primary
    geometric midpoint for that floc band. S(q) is a mass-fractal structure
    factor using the aggregate radius of gyration derived from the existing
    fractal mass model.
    """
    source_mid = np.sqrt(
        source_primary_min_diameter_m[bin_idx] *
        source_primary_max_diameter_m[bin_idx]
    )
    source_mid = max(source_mid, 1.0e-30)
    ...
    monomer_count = (
        particle_mass_by_bin_kg[bin_idx] / primary_mass_mid
        if primary_mass_mid > 0.0
        else 1.0
    )
    monomer_count = max(float(monomer_count), 1.0)

    monomer_radius = 0.5 * source_mid

    # Fractal relation N ~= (Rg/a)^Df with prefactor fixed to unity to avoid
    # adding a new fit parameter. Bound Rg by the configured effective floc radius
    # so the structure factor remains tied to the chosen aggregate diameter.
    rg_from_fractal = monomer_radius * (monomer_count ** (1.0 / FLOC_FRACTAL_DIMENSION))
    rg_from_diameter = 0.5 * particle_diameter_m[bin_idx]
    rg_m = min(max(rg_from_fractal, monomer_radius), rg_from_diameter)

    q_values = (
        4.0 * np.pi * n_medium / wl *
        np.sin(0.5 * theta_rad_values)
    )

    primary_form = representative_primary_mie_intensity(
        theta_rad_values,
        wl,
        source_mid
    )
    structure_factor = rdgfa_structure_factor(
        q_values,
        rg_m,
        FLOC_FRACTAL_DIMENSION,
        monomer_count
    ...
    intensity = primary_form * structure_factor
    return np.maximum(np.real(intensity), 1.0e-300).astype(np.float64)
...
if os.path.exists(mie_cache_file):
    ...
    print("Building angular tables: primary Mie + floc RDG-FA diagnostics...")
    ...
    for wl in wavelengths:
        ...
        for bin_idx, (radius, weight, refr_index, is_floc) in enumerate(zip(
            ...
            if is_floc:
                # Flocs are not Mie spheres in CLARITAS 3.1.
                # Use an RDG-FA style aggregate phase function:
                # representative primary-material form factor x fractal
                # aggregate structure factor.
                I = rdgfa_floc_intensity_profile(
                    theta_rad,
                    wl,
                    bin_idx
                )
...
print(f"OK Saved {csv_angles_path}")
...
plt.title("Angular scattering profiles - primary Mie + floc RDG-FA")
...
print("OK Saved angular_scattering_profiles.png")
    ...
    unsigned int seed2)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            // Primary and floc bins both use the host-built angular CDF table.
            // Primary rows contain Mie CDFs. Floc rows contain RDG-FA aggregate
            // CDFs generated from representative primary form factor x structure factor.
...
def save_detector_geometry_diagnostics(
    ...
    print(f"OK Saved {scatter_hist_csv_path}")
    print(f"OK Saved {scatter_hist_png_path}")
    print(f"OK Saved {raw_exit_csv_path}")
...
#        norm=LogNorm(vmin=1, vmax=np.max(heatmap)),
    ...
    print(f"OK Saved {exits_csv_path}")
...
plt.close()
print(f"OK Saved detector_hits.png")
...
plt.close()
print(f"OK Saved detector_hits_polar.png")
...
plt.close()
print("OK Simulation complete. Heatmaps + bulk angular scattering + detector hits + exit point plots saved.")
...
plt.close()
print(f"OK Saved {wavelength_response_path}")
...
plt.close()
print(f"OK Saved {polar_response_path}")
...
plt.close()
print(f"OK Saved log-scale histogram plot as {hist_png_path}")
...
print(f"OK Saved histogram data to {hist_csv_path}")
...
# Total signal (0-180 deg)
total = np.sum(counts * dtheta)

# Backscatter region (90-180 deg)
mask_back = angles >= 90
...
if FLOC_ENABLED and len(FLOC_POOL_PRIMARY_BAND_MAX_DIAMETER_M) > 0:
    ...
    print(f"OK Saved {floc_band_summary_path}")
...
print(f"OK Saved {psd_compare_csv}")
...
plt.bar(
    ...
    label=f"Original primary PSD ({particle_density_kg_per_m3:.0f} kg/m^3)"
)
...
print(f"OK Saved {psd_compare_png}")
...
print(f"OK Saved {floc_diag_csv}")
...
print(f"OK Saved {floc_diag_png_1}")
...
plt.ylabel("Density used in model (kg/m^3)")
...
print(f"OK Saved {floc_diag_png_2}")
...
print(f"OK Saved {floc_diag_png_3}")
...
print(f"OK Saved {ri_diag_png}")

print("STEP_SIZE =", STEP_SIZE)
print("scatter_prob_per_step =", scatter_prob_per_step)
print("R_REAL =", R_REAL)
...
print(f"OK Saved {event_diagnostics_path}")
...
print(f"OK Saved {floc_diag_path}")
...
if np.any(particle_is_floc):
    ...
    print(f"OK Saved {floc_event_path}")
else:
    print("No floc bins present; skipped floc_event_weight_by_band diagnostic.")

    # ================= RDG-FLOC ANGULAR DIAGNOSTICS =================

if np.any(particle_is_floc):
    diagnostic_angles_deg = np.array([
        0.0, 10.0, 20.0, 30.0, 60.0, 90.0, 120.0, 150.0, 160.0, 170.0
    ])

    diagnostic_theta_idx = np.array([
        closest_index(theta_deg, a)
        for a in diagnostic_angles_deg
    ])

    rdg_diag_rows = []

    # cdf_profiles[0] is [effective_bin, theta] for first wavelength.
    cdf_table_first_wl = cdf_profiles[0]

    for bin_idx in np.where(particle_is_floc)[0]:
        profile = np.diff(
            np.concatenate([[0.0], cdf_table_first_wl[bin_idx]])
        )

        profile = np.maximum(profile, 0.0)

        if np.max(profile) > 0.0:
            profile_norm_peak = profile / np.max(profile)
        else:
            profile_norm_peak = profile

        row = {
            "effective_bin_index": bin_idx,
            "floc_band_index": floc_band_index_by_effective_bin[bin_idx],
            "effective_diameter_um": particle_diameter_m[bin_idx] * 1.0e6,
            "source_primary_min_um": source_primary_min_diameter_m[bin_idx] * 1.0e6,
            "source_primary_max_um": source_primary_max_diameter_m[bin_idx] * 1.0e6,
            "source_primary_mid_um": (
                np.sqrt(
                    source_primary_min_diameter_m[bin_idx] *
                    source_primary_max_diameter_m[bin_idx]
            ...
            "mass_fraction": particle_weights[bin_idx],
            "event_probability": particle_event_weights[bin_idx],
            "fraction_of_all_floc_events": (
                particle_event_weights[bin_idx] / floc_event_probability
                if floc_event_probability > 0.0 else 0.0
            ),
            "sigma_s_m2": sigma_s[bin_idx],
            "mu_s_by_bin_per_m": mu_s_by_bin[bin_idx],
            "g_by_bin": g_by_bin[bin_idx],
        }

        for angle, idx in zip(diagnostic_angles_deg, diagnostic_theta_idx):
            row[f"I_peaknorm_{angle:.0f}deg"] = profile_norm_peak[idx]
            row[f"CDF_{angle:.0f}deg"] = cdf_table_first_wl[bin_idx, idx]

        i0 = profile_norm_peak[closest_index(theta_deg, 0.0)]
        i90 = profile_norm_peak[closest_index(theta_deg, 90.0)]
        i170 = profile_norm_peak[closest_index(theta_deg, 170.0)]

        row["I170_over_I0_peaknorm"] = (
            i170 / i0 if i0 > 0.0 else np.nan
        )

        row["I90_over_I0_peaknorm"] = (
            i90 / i0 if i0 > 0.0 else np.nan
        )

        rdg_diag_rows.append(row)

    rdg_floc_diag_df = pd.DataFrame(rdg_diag_rows)

    rdg_floc_diag_path = os.path.join(
        OUTDIR,
        f"rdg_floc_angular_diagnostics_conc_{mass_concentration_g_per_L}.csv"
    )

    rdg_floc_diag_df.to_csv(
        rdg_floc_diag_path,
        index=False
    )

    print(f"✅ Saved {rdg_floc_diag_path}")
else:
    print("No floc bins present; skipped RDG floc angular diagnostics.")