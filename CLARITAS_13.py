...
# Process-based ray tracer with diameter-dependent effective floc PSD and dual reflection controls.
...
#   - Only those floc bins receive the lower floc effective density.
...
FLOC_ENABLED = True
FLOC_PRIMARY_MAX_DIAMETER = 20.0e-6
...
FLOC_DIAMETER_MULTIPLIER_EXPONENT = 0.6

FLOC_EFFECTIVE_DENSITY_KG_PER_M3 = 300.0
...
FLOC_REFLECT_PROB = 0.15
FLOC_REFLECT_SIZE_THRESHOLD = 70.0e-6
...
if FLOC_ENABLED:
    ...
    # Only floc bins get the lower effective density.
...
# Geometric cross-sectional loading, without Mie efficiency.
# Units: 1/m. Useful diagnostic but not used directly as per-step probability.
mu_geom = np.sum(
    particle_number_density_by_bin *
    np.pi *
    particle_radius_m**2
...
# Whole-sample optical-depth-like loading.
# This is dimensionless and should NOT be used directly as per-step probability.
# For a central beam through the circular sample, L_eff = 2R.
# For random chords, 4R/pi is often more appropriate.
# PSD_RESPONSE_BOOST / SCATTER_2D_BOOST is a fixed empirical 2D-transport calibration factor.
# At 0.30 it reduces the full 3D Mie scattering rate by about 3.3x, matching
# the observation that the uncorrected per-step probability was 3-4x too large.
L_eff = 2.0 * R_REAL
...
print(f"FLOC_EFFECTIVE_DENSITY_KG_PER_M3: {FLOC_EFFECTIVE_DENSITY_KG_PER_M3:.3e}")
...
mie_cache_version = "per_particle_intensity_no_sin_theta_diameter_dependent_flocs_v1"
...
mie_cache_key = (
    ...
    str(FLOC_EFFECTIVE_DENSITY_KG_PER_M3) +
...
plt.bar(
    ...
    alpha=0.85,
    color="tab:red",
    label=f"Effective PSD (floc bins, {FLOC_EFFECTIVE_DENSITY_KG_PER_M3:.0f} kg/m³)"
)
