#!/usr/bin/env python3
"""Shared CLARITAS V24.3 / TARDIIS apparatus forward model.
...
V24.3 leaves the V24 CUDA optical transport unchanged. It adds streaming exact
Monte Carlo detector accumulation and an unbiased left/right symmetry average.
...
def detect_physical_apertures(x,y,z,vx,vy,vz,valid,inner_plane_radius_m,outer_plane_radius_m,aperture_radius_m):
    """Exact two-plane physical TARDIIS aperture scoring.

    Each ray is assigned to at most one detector. There is no widened aperture,
    KDE, angular tolerance, interpolation or extrapolation in this routine.
    """
    ...
    rr=float(aperture_radius_m)*float(aperture_radius_m)
    for j,deg in enumerate(DETECTOR_ANGLES_DEG):
        ...
        t1[good]=(inner_plane_radius_m-pdot[good])/den[good]
        t2[good]=(outer_plane_radius_m-pdot[good])/den[good]
        good &= (t1>0)&(t2>t1)
        if not np.any(good): continue
        ids=np.flatnonzero(good)
        q1=p[ids]+t1[ids,None]*v[ids]-inner_plane_radius_m*u[None,:]
        q2=p[ids]+t2[ids,None]*v[ids]-outer_plane_radius_m*u[None,:]
        aperture=(np.einsum('ij,ij->i',q1,q1)<=rr) & (np.einsum('ij,ij->i',q2,q2)<=rr)
        if not np.any(aperture): continue
        hit_ids=ids[aperture]
        upd=t1[hit_ids]<best_t[hit_ids]
        hit_ids=hit_ids[upd]
        best_t[hit_ids]=t1[hit_ids];best[hit_ids]=j
    hits=np.bincount(best[best>=0],minlength=DETECTOR_ANGLES_DEG.size).astype(np.int64)
    return hits,best


def detect_symmetry_averaged_exact(x,y,z,vx,vy,vz,valid,inner_plane_radius_m,outer_plane_radius_m,aperture_radius_m):
    """Unbiased exact-aperture variance reduction from model reflection symmetry.

    The V24 TARDIIS model is invariant under x -> -x: the cylindrical cell,
    homogeneous suspension, source distribution and optical laws are mirror
    symmetric about the y-z plane.  For each simulated ray X, M(X) is therefore
    an equally distributed ray.  We score BOTH X and its mirror with the same
    hard two-plane physical aperture test and use 0.5*(H(X)+H(MX)).

    This is a Rao-Blackwell/antithetic symmetry average, not detector widening:
    every branch still either passes through both real 4-mm apertures or it does
    not.  Native one-sided counts are retained separately for audit/regression.
    """
    native,native_id=detect_physical_apertures(
        x,y,z,vx,vy,vz,valid,inner_plane_radius_m,outer_plane_radius_m,aperture_radius_m)
    mirror,mirror_id=detect_physical_apertures(
...
class ForwardResult:
    ...
    raw_hits: np.ndarray                       # native one-sided hard-aperture hits
    normalized_response: np.ndarray            # selected: symmetry-averaged exact
    native_exact_hits: np.ndarray
    mirror_exact_hits: np.ndarray
    symmetry_exact_scores: np.ndarray
...
class TardiisForwardModel:
    def __init__(self,n_water=1.33,n_particle=1.59,n_acrylic=1.4906,n_air=1.0,
        ...
        except Exception as exc: raise RuntimeError('CuPy/CUDA is required for CLARITAS V24.3.') from exc
    ...
    def simulate(self,material,concentration_g_per_L,weights=None,n_rays=100000,seed=12345,
        ...
        """Run the unchanged V24 CUDA transport with streaming exact detector scoring.
        ...
        No KDE / widened detector / extrapolation is used.
        """
        ...
        batch_scores=[];batch_rows=[];prev_cum=None;recent_l1=[]
        ...
        while total_rays<maxN:
            ...
            for local_start in range(0,batchN,self.chunk_size):
                ...
                nat,mir,sym,nid,mid=detect_symmetry_averaged_exact(
                    arrays['air_x'],arrays['air_y'],arrays['air_z'],arrays['air_vx'],arrays['air_vy'],arrays['air_vz'],valid_air,
                    self.coll_in,self.coll_out,self.aperture_radius)
            ...
            sym_total+=batch_sym;batch_scores.append(batch_sym.copy())
        ...
        se=_jackknife_normalized_se(batch_scores)
        ...
        return ForwardResult(
            ...
            normalized_response_jackknife_se=se,valid_exit_count=valid_air_total,n_rays=total_rays,requested_n_rays=requested,
            ...
            symmetry_variance_reduction=self.symmetry_average_exact,batch_diagnostics=batch_rows,ray_data=ray_data,heatmap_xy=hmxy,heatmap_zy=hmzy)
