#!/usr/bin/env python3
# CLARITAS_64_true_floc_domain.py
...
mie_cache_version = "claritas_6p4_true_cuda_floc_domain_v1"
    ...
    unsigned int seed2)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (is_floc_event && FLOC_CUDA_TRUE_INTERNAL_DOMAIN_ENABLED) {
            // CLARITAS_64 true internal floc-domain transport.
            // A floc event is no longer one pre-shaped phase-function sample.
            // Instead, draw a raw internal scattering optical depth and perform
            // a sequence of representative-monomer angular scatters before the
            // photon returns to the bulk suspension.
            ...
            while (internal_scatter_count < FLOC_INTERNAL_DOMAIN_MAX_SCATTERS) {
                float u_tau = fmaxf(rnd_uniform(&state), 1.0e-12f);
                tau_accum += -logf(u_tau);
...
def trace_rays_gpu(angles_init_np, primary_roughness_std_rad, floc_roughness_std_rad,
                   ...
                   internal_angle_cdf_table_np, floc_internal_tau_s_table_np, theta_table_np,
