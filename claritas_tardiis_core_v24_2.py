#!/usr/bin/env python3
"""Shared CLARITAS V24.2 / TARDIIS apparatus forward model.

V24.2 changes detector *estimation*, not optical physics.  It replaces the
single fixed-bandwidth V24.1 detector KDE by a multi-bandwidth estimator and a
per-detector zero-bandwidth extrapolation.  In addition to the
...
- multi-bandwidth variance-reduced detector scoring from a product
  Epanechnikov kernel in the two transverse aperture planes;
- physical-equivalent scores at several bandwidths are extrapolated linearly
  in h^2 to h -> 0, while exact physical hits remain available for audit;
...
def score_physical_apertures_multibandwidth(
    x, y, z, vx, vy, vz, valid,
    inner_plane_radius_m, outer_plane_radius_m, aperture_radius_m,
    bandwidth_factors=(2.0, 3.0, 4.0), extrapolation_power=2.0,
):
    """Exact TARDIIS collimator hits plus multi-bandwidth phase-space scores.

    For each detector, the same physical ray exits are projected to the two
    circular collimator planes.  The exact score remains a binary requirement
    that both intersections fall inside the real aperture radius ``a``.

    Variance-reduced scores are calculated at several fixed bandwidths
    ``h_k = bandwidth_factor_k * a`` using the product of two 2-D Epanechnikov
    kernels::

        K_k = max(0, 1-|q1|^2/h_k^2) * max(0, 1-|q2|^2/h_k^2)

    Each bandwidth is independently normalised to the physical two-aperture
    acceptance volume::

        H_k = sum(K_k) * 4 * (a/h_k)^4

    For a symmetric local KDE, the leading smoothing bias is O(h^2).  V24.2
    therefore fits, independently for every detector,

        H(h) = H0 + A * (h/a)^p

    with default ``p=2`` and equal weight for each configured bandwidth.  The
    intercept H0 is the zero-bandwidth estimate.  The raw intercept is retained
    for diagnostics; the production physical score uses ``max(H0, 0)`` because
    detector irradiance cannot be negative.

    This estimator operates only after all optical transport is complete.  It
    does not modify any ray, probability, Fresnel decision, particle encounter,
    or apparatus geometry.
    """
    ...
    bfs = np.asarray(tuple(bandwidth_factors), dtype=np.float64)
    if bfs.ndim != 1 or bfs.size < 2 or np.any(~np.isfinite(bfs)) or np.any(bfs < 1.0):
        raise ValueError('detector VR bandwidth_factors must contain at least two finite values >= 1.0')
    if np.any(np.diff(bfs) <= 0):
        raise ValueError('detector VR bandwidth_factors must be strictly increasing')
    power = float(extrapolation_power)
    if not np.isfinite(power) or power <= 0:
        raise ValueError('detector extrapolation_power must be finite and > 0')
    ...
    kernel_sums = np.zeros((n_bw, n_det), dtype=np.float64)
    support_counts = np.zeros((n_bw, n_det), dtype=np.int64)

    # Exact physical detector assignment is exclusive, preserving V24/V24.1.
    # KDE values are evaluations of local phase-space density and therefore do
    # not need to be exclusive between detector centres.
    ...
    for j, deg in enumerate(DETECTOR_ANGLES_DEG):
        ...
        for bi in range(n_bw):
            sup = (r1sq < h2[bi]) & (r2sq < h2[bi])
            support_counts[bi, j] = int(np.count_nonzero(sup))
            if np.any(sup):
                k1 = 1.0 - r1sq[sup]/h2[bi]
                k2 = 1.0 - r2sq[sup]/h2[bi]
                kernel_sums[bi, j] = float(np.sum(k1*k2, dtype=np.float64))

    if np.any(best >= 0):
        exact_hits = np.bincount(best[best >= 0], minlength=n_det).astype(np.int64)

    # Physical-equivalent scale for each bandwidth.
    scales = 4.0 / (bfs**4)
    physical_equivalent = kernel_sums * scales[:, None]

    # Equal-weight local linear bias extrapolation against (h/a)^power.  Since
    # a is common, using bandwidth_factor**power gives the same intercept as h^p.
    xx = bfs**power
    xbar = float(np.mean(xx))
    dx = xx - xbar
    denom = float(np.sum(dx*dx))
    if denom <= 0:
        raise ValueError('bandwidth factors do not provide a usable extrapolation basis')
    ybar = np.mean(physical_equivalent, axis=0)
    slope = np.sum(dx[:, None]*(physical_equivalent-ybar[None, :]), axis=0) / denom
    intercept_raw = ybar - slope*xbar
    prediction = intercept_raw[None, :] + slope[None, :]*xx[:, None]
    ss_res = np.sum((physical_equivalent-prediction)**2, axis=0)
    ss_tot = np.sum((physical_equivalent-ybar[None, :])**2, axis=0)
    fit_r2 = np.ones(n_det, dtype=np.float64)
    nz = ss_tot > 1.0e-30
    fit_r2[nz] = 1.0 - ss_res[nz]/ss_tot[nz]
    fit_r2[~np.isfinite(fit_r2)] = np.nan

    # Physical non-negativity.  Raw intercept is always retained for diagnostics.
    intercept_physical = np.maximum(intercept_raw, 0.0)
    mean_abs = np.maximum(np.abs(ybar), 1.0e-15)
    relative_spread = (np.max(physical_equivalent,axis=0)-np.min(physical_equivalent,axis=0)) / mean_abs

    return (exact_hits, best, bfs, physical_equivalent, kernel_sums, support_counts,
            intercept_raw, intercept_physical, slope, fit_r2, relative_spread)
...
def score_physical_apertures_variance_reduced(
    ...
    """V24.1 compatibility wrapper for one fixed bandwidth.

    This wrapper intentionally bypasses zero-bandwidth extrapolation and returns
    the same five values as V24.1.  New code should use
    :func:`score_physical_apertures_multibandwidth`.
    """
    # Use the original fixed-bandwidth calculation directly so a one-bandwidth
    # diagnostic remains available without violating the multi-bandwidth API.
    p = np.column_stack([x, y, z]).astype(np.float64, copy=False)
    v = np.column_stack([vx, vy, vz]).astype(np.float64, copy=False)
    valid = np.asarray(valid, dtype=bool)
    a=float(aperture_radius_m); bf=float(bandwidth_factor)
    if not np.isfinite(bf) or bf < 1.0: raise ValueError('bandwidth_factor must be >=1')
    h=a*bf; a2=a*a; h2=h*h; n_det=DETECTOR_ANGLES_DEG.size
    exact_hits=np.zeros(n_det,dtype=np.int64); kernel_sum=np.zeros(n_det); support=np.zeros(n_det,dtype=np.int64)
    best_t=np.full(p.shape[0],np.inf); best=np.full(p.shape[0],-1,dtype=np.int16)
    for j,deg in enumerate(DETECTOR_ANGLES_DEG):
        th=np.deg2rad(deg); u=np.array([np.sin(th),np.cos(th),0.0]); den=v@u; pdot=p@u
        good=valid&(den>1e-10)
        if not np.any(good): continue
        t1=np.full(p.shape[0],np.nan); t2=np.full(p.shape[0],np.nan)
        t1[good]=(float(inner_plane_radius_m)-pdot[good])/den[good]
        t2[good]=(float(outer_plane_radius_m)-pdot[good])/den[good]
        good&=(t1>0)&(t2>t1); ids=np.flatnonzero(good)
        if ids.size==0: continue
        q1=p[ids]+t1[ids,None]*v[ids]-float(inner_plane_radius_m)*u
        q2=p[ids]+t2[ids,None]*v[ids]-float(outer_plane_radius_m)*u
        r1=np.einsum('ij,ij->i',q1,q1); r2=np.einsum('ij,ij->i',q2,q2)
        ex=(r1<=a2)&(r2<=a2)
        if np.any(ex):
            eid=ids[ex]; upd=t1[eid]<best_t[eid]; chosen=eid[upd]; best_t[chosen]=t1[chosen]; best[chosen]=j
        sup=(r1<h2)&(r2<h2); support[j]=np.count_nonzero(sup)
        if np.any(sup): kernel_sum[j]=np.sum((1-r1[sup]/h2)*(1-r2[sup]/h2))
    if np.any(best>=0): exact_hits=np.bincount(best[best>=0],minlength=n_det).astype(np.int64)
    eq=kernel_sum*(4.0/(bf**4))
    return exact_hits,best,eq,kernel_sum,support
...
class ForwardResult:
    ...
    normalized_multiband_extrapolated_response: np.ndarray
    multiband_extrapolated_physical_equivalent_scores: np.ndarray
    multiband_extrapolated_raw_scores: np.ndarray
    multiband_bandwidth_factors: np.ndarray
    multiband_physical_equivalent_scores: np.ndarray
    multiband_normalized_responses: np.ndarray
    multiband_kernel_sums: np.ndarray
    multiband_support_counts: np.ndarray
    multiband_fit_slope: np.ndarray
    multiband_fit_r2: np.ndarray
    multiband_relative_spread: np.ndarray
    detector_scoring_mode: str
    detector_vr_bandwidth_factors: np.ndarray
    detector_extrapolation_power: float
    ...
    detector_multiband_equivalent_fraction: float # extrapolated physical-equivalent sum / launched rays
...
class TardiisForwardModel:
    def __init__(self,n_water=1.33,n_particle=1.59,n_acrylic=1.4906,n_air=1.0,
        ...
        except Exception as exc: raise RuntimeError('CuPy/CUDA is required for CLARITAS V24.2.') from exc
        ...
        if self.detector_scoring_mode not in {'multiband_extrapolated','smallest_bandwidth','exact'}:
            raise ValueError("detector_scoring_mode must be multiband_extrapolated, smallest_bandwidth, or exact")
        self.detector_vr_bandwidth_factors=np.asarray(detector_vr_bandwidth_factors,dtype=np.float64)
        if self.detector_vr_bandwidth_factors.ndim!=1 or self.detector_vr_bandwidth_factors.size<2 or np.any(self.detector_vr_bandwidth_factors<1.0) or np.any(np.diff(self.detector_vr_bandwidth_factors)<=0):
            raise ValueError('detector_vr_bandwidth_factors must be a strictly increasing sequence of at least two values >=1')
        self.detector_extrapolation_power=float(detector_extrapolation_power)
        if not np.isfinite(self.detector_extrapolation_power) or self.detector_extrapolation_power<=0:
            raise ValueError('detector_extrapolation_power must be >0')
    ...
    def simulate(self,material,concentration_g_per_L,weights=None,n_rays=100000,seed=12345,weight_mode='mass_fraction',collect_rays=False,heatmap_size=0):
        ...
        (hits,det_id,bw_factors,mb_eq,mb_kernel,mb_support,extrap_raw,extrap_phys,fit_slope,fit_r2,rel_spread)=score_physical_apertures_multibandwidth(
            arrays['air_x'],arrays['air_y'],arrays['air_z'],arrays['air_vx'],arrays['air_vy'],arrays['air_vz'],
            valid_air,self.coll_in,self.coll_out,self.aperture_radius,self.detector_vr_bandwidth_factors,self.detector_extrapolation_power)
        ...
        mb_totals=np.sum(mb_eq,axis=1)
        mb_norm=np.divide(mb_eq,mb_totals[:,None],out=np.zeros_like(mb_eq),where=mb_totals[:,None]>0)
        extrap_total=float(extrap_phys.sum())
        extrap_norm=extrap_phys/extrap_total if extrap_total>0 else np.zeros_like(extrap_phys,dtype=np.float64)
        if self.detector_scoring_mode == 'exact':
            selected_scores=hits.astype(np.float64); norm=exact_norm
        elif self.detector_scoring_mode == 'smallest_bandwidth':
            selected_scores=mb_eq[0].astype(np.float64,copy=False); norm=mb_norm[0]
        else:
            selected_scores=extrap_phys.astype(np.float64,copy=False); norm=extrap_norm
        ...
        return ForwardResult(
            ...
            normalized_exact_response=exact_norm,normalized_multiband_extrapolated_response=extrap_norm,
            multiband_extrapolated_physical_equivalent_scores=extrap_phys,
            multiband_extrapolated_raw_scores=extrap_raw,multiband_bandwidth_factors=bw_factors,
            multiband_physical_equivalent_scores=mb_eq,multiband_normalized_responses=mb_norm,
            multiband_kernel_sums=mb_kernel,multiband_support_counts=mb_support,
            multiband_fit_slope=fit_slope,multiband_fit_r2=fit_r2,multiband_relative_spread=rel_spread,
            detector_scoring_mode=self.detector_scoring_mode,
            detector_vr_bandwidth_factors=self.detector_vr_bandwidth_factors.copy(),
            detector_extrapolation_power=self.detector_extrapolation_power,
            ...
            detector_detection_fraction=float(exact_total/N),detector_multiband_equivalent_fraction=float(extrap_total/N),
