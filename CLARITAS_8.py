...
RELFECT_SIZE_THRESHOLD = 0.0  # minimum diameter (in CDF) for reflection to occur
...
N_RAYS = 2_000_00  # number of rays to simulate
...
#samp_csa = dist_csa ** (1.0/3.0) * np.pi * R_REAL**2 * n_particles_per_m3 ** (1.0/3.0) 

#samp_csa = 1.5e-3

# Physically meaningful sample cross-sectional loading:
# tau_geom = projected particle area per unit volume × optical path length
#          = L_eff * Σ(n_i * π r_i²)
...
# Treat the supplied PSD weights as mass/volume fractions for particle counting.
# Number density per bin is therefore concentration fraction divided by mass per particle.
...
L_eff = 2.0 * R_REAL  # central chord through circular sample; use mean_chord below if needed
...
samp_csa = L_eff * np.sum(
    particle_number_density_by_bin * np.pi * particle_radius_m**2
)
...
# Total scattering coefficient: sum over each bin's interaction rate.
...
# 3D -> 2D empirical correction with PSD cross-sectional-area loading.
# Keep STEP_SIZE in the exponent; tune SCATTER_2D_BOOST if needed.
# samp_csa provides a PSD/concentration-dependent cross-sectional loading factor,
# while SCATTER_2D_BOOST remains a fixed instrument/model calibration factor.

#if samp_csa > 0.0 and SAMP_CSA_REF > 0.0:
#    csa_scatter_gain = (samp_csa / SAMP_CSA_REF) ** CSA_SCATTER_EXPONENT
#else:
#    csa_scatter_gain = 0.0

#scatter_prob_per_step = samp_csa ** (CSA_SCATTER_EXPONENT)
scatter_prob_per_step = samp_csa *0.00012
#scatter_prob_per_step = (mass_concentration_g_per_L / 4.0) * dist_csa ** CSA_SCATTER_EXPONENT
scatter_prob_per_step = min(scatter_prob_per_step, 1.0)

#reflection_path_length = STEP_SIZE / scatter_prob_per_step
reflection_path_length = 0

#scatter_prob_per_step = 1.0 - np.exp(
#    -mu_s * STEP_SIZE * SCATTER_2D_BOOST * csa_scatter_gain
#)
...
mie_cache_version = "per_particle_no_sin_theta_v2"
...
//                }
        ...
        // ----------------------------
        // Boundary Fresnel handling
        // ----------------------------
        float x_next = x + vx * STEP_SIZE;
        float y_next = y + vy * STEP_SIZE;

        if (x_next * x_next + y_next * y_next > R_REAL * R_REAL) {

            // Outward surface normal at the current boundary-side point.
            float nr = sqrtf(x * x + y * y);
            float nx = x / nr;
            float ny = y / nr;

            // Direction cosine relative to outward normal.
            float cos_i = vx * nx + vy * ny;

            if (cos_i > 0.0f) {

                float eta = N_MEDIUM / N_EXTERNAL;
                float sin2_t = eta * eta * fmaxf(0.0f, 1.0f - cos_i * cos_i);
                ...
                if (sin2_t <= 1.0f) {
                    ...
                    float rs_num = N_MEDIUM * cos_i - N_EXTERNAL * cos_t;
                    float rs_den = N_MEDIUM * cos_i + N_EXTERNAL * cos_t;
                    float rp_num = N_MEDIUM * cos_t - N_EXTERNAL * cos_i;
                    float rp_den = N_MEDIUM * cos_t + N_EXTERNAL * cos_i;

                    float rs = (rs_num / rs_den) * (rs_num / rs_den);
                    float rp = (rp_num / rp_den) * (rp_num / rp_den);

                    reflectance = 0.5f * (rs + rp);
                }

                if (rnd_uniform(&stateREFL) < reflectance) {
                    // Specular internal reflection.
                    vx = vx - 2.0f * cos_i * nx;
                    vy = vy - 2.0f * cos_i * ny;

                    // Nudge ray inward to avoid sticking to the boundary.
                    x = x - nx * STEP_SIZE * 0.25f;
                    y = y - ny * STEP_SIZE * 0.25f;
                } else {
                    // Transmit/exit the sample.
                    x = x_next;
                    y = y_next;
                }
            } else {
                // Numerical fallback: if not moving outward, continue normally.
                x = x_next;
                y = y_next;
            }
        } else {
