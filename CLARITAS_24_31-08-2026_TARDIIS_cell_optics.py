#!/usr/bin/env python3
"""CLARITAS V24: TARDIIS-specific 3-D cylindrical-cell optical transport.

Particle physics remains the successful Mie-free V23.2 model: geometric
sphere encounters plus full 3-D Snell/Fresnel physics.  V24 replaces the
artificial spherical sample boundary and abstract detector caps with the
actual TARDIIS radial apparatus: water cylinder, acrylic tube wall, air gap,
...
from claritas_tardiis_core_v24 import TardiisForwardModel, DETECTOR_ANGLES_DEG, get_material_psd, build_geometric_transport
...
def save_geometry_plot(outdir: Path, geom: dict):
    ...
    ax.set_title('CLARITAS V24 TARDIIS radial optical geometry')
...
def main():
    ap=argparse.ArgumentParser(description='CLARITAS V24 TARDIIS cylindrical-cell geometric/Fresnel transport')
    ...
    ap.add_argument('--output-dir',help='default: claritas_v24_results/<material>_<concentration>gL')
    ...
    outdir=Path(args.output_dir or f'claritas_v24_results/{tag}'); outdir.mkdir(parents=True,exist_ok=True)
    model,geom=make_model(args)
    print('\n=========== CLARITAS V24 — TARDIIS CELL OPTICS ===========')
    ...
    print('Detector physics: physical two-plane 4-mm collimator aperture test; one detector max per ray')
    ...
    pd.DataFrame({'Detector_deg':res.detector_angles_deg.astype(int),'raw_hits':res.raw_hits}).to_csv(outdir/'detector_hits.csv',index=False)
    pd.DataFrame({'Detector_deg':res.detector_angles_deg.astype(int),'normalized_response':res.normalized_response}).to_csv(outdir/'detector_response_normalized.csv',index=False)
    ...
    fig,ax=plt.subplots(figsize=(9,5)); ax.plot(res.detector_angles_deg,res.normalized_response,'o-')
    ax.set_xlabel('Detector angle (deg)'); ax.set_ylabel('Normalized detector response'); ax.set_xticks(DETECTOR_ANGLES_DEG)
    ax.set_title(f'CLARITAS V24 TARDIIS response — {tag}'); ax.grid(True,alpha=.25); fig.tight_layout(); fig.savefig(outdir/'detector_response_normalized.png',dpi=200); plt.close(fig)
    ...
    print(f'Saved V24 results to {outdir}')
    print(f'Detected ray fraction: {res.detector_detection_fraction:.6g}')
