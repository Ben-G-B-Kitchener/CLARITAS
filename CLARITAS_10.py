...
# Process-based ray tracer with concentration-dependent floc PSD and dual reflection controls.
...
# ============================ FLOCCULATION MODEL ============================
# Flocculation is computed host-side by converting the selected primary sediment
# PSD into an effective concentration-dependent PSD before number densities,
# Mie tables, event weights, and CUDA lookup tables are built.
FLOC_ENABLED = True
FLOC_PRIMARY_MAX_DIAMETER = 10.0e-6
FLOC_REF_CONC_G_PER_L = 0.5
FLOC_GROWTH_ALPHA = 0.20
FLOC_MAX_GROWTH = 8.0
FLOC_MIN_GROWTH = 1.0
#FLOC_EFFECTIVE_DENSITY_KG_PER_M3 = None  # None keeps selected sediment density
FLOC_EFFECTIVE_DENSITY_KG_PER_M3 = 300.0  # None keeps selected sediment density
...
if FLOC_ENABLED:
    floc_growth = (
        mass_concentration_g_per_L / FLOC_REF_CONC_G_PER_L
    ) ** FLOC_GROWTH_ALPHA
    floc_growth = np.clip(floc_growth, FLOC_MIN_GROWTH, FLOC_MAX_GROWTH)
    ...
    particle_diameter_m[particle_is_floc] *= floc_growth

    effective_particle_density_kg_per_m3 = (
        particle_density_kg_per_m3
        if FLOC_EFFECTIVE_DENSITY_KG_PER_M3 is None
        else FLOC_EFFECTIVE_DENSITY_KG_PER_M3
    )
else:
    floc_growth = 1.0
    particle_is_floc = np.zeros_like(primary_particle_diameter_m, dtype=bool)
    particle_diameter_m = primary_particle_diameter_m.copy()
    effective_particle_density_kg_per_m3 = particle_density_kg_per_m3
...
particle_masses = particle_volumes * effective_particle_density_kg_per_m3
...
print(f"floc_growth: {floc_growth:.3f}")
...
print(f"effective_particle_density_kg_per_m3: {effective_particle_density_kg_per_m3:.3e}")
...
mie_cache_version = "per_particle_intensity_no_sin_theta_floc_v1"
...
mie_cache_key = (
    ...
    str(FLOC_REF_CONC_G_PER_L) +
    str(FLOC_GROWTH_ALPHA) +
    str(FLOC_MAX_GROWTH) +
    str(FLOC_MIN_GROWTH) +
    str(effective_particle_density_kg_per_m3)
)
