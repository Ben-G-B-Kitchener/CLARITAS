#!/usr/bin/env python3
# CLARITAS_23_2_30-08-2026_fully_3d_fresnel_normalized_csv.py
#
...
# Explicit normalized detector-response CSV.  This uses exactly the same
# normalization as detector_response_vs_wavelength.png: each wavelength's
# detector hit-count vector is divided by the sum across detector bins.
# Note that detector acceptance caps can overlap, so these are normalized
# detector responses rather than mutually exclusive per-ray probabilities.
df_det_norm = pd.DataFrame({"Detector_deg": detector_angles})
for wl_nm, counts in detector_hit_counts.items():
    ...
    df_det_norm[f"H_{wl_nm}nm_normalized"] = counts_norm

det_norm_csv_path = os.path.join(OUTDIR, "detector_response_normalized.csv")
df_det_norm.to_csv(det_norm_csv_path, index=False)
print(f"✅ Saved {det_norm_csv_path}")
...
plt.figure(figsize=(9,5))
for wl_nm in detector_hit_counts:
    counts_norm = df_det_norm[f"H_{wl_nm}nm_normalized"].to_numpy()
...
print("\n=========== V23.2 FULLY 3-D FRESNEL PHYSICS SUMMARY ===========")
