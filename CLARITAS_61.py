...
mie_cache_version = "claritas_6p2_aggregate_scale_floc_phase_v1"
...
def floc_synthetic_structure_profile(bin_idx, wavelength_m, mu_values, theta_rad_values):
    """
    CLARITAS_62 replacement floc phase function.

    Keeps CLARITAS_61 transport/PSD unchanged, but prevents the floc phase
    from collapsing back to representative-primary Mie when S(q) ~= 1.

    Uses:
      - aggregate-scale diffraction from the whole floc diameter
      - representative-primary internal component
      - structure-correlated internal component
      - optional internal-transport broadening from existing CLARITAS_60 helper
    """
    n_synth, n_physical, rep_idx = _representative_monomer_count_for_floc_bin(bin_idx)
    ...
    # Aggregate-scale diffraction from the full floc size, not the monomer size.
    floc_diameter = float(particle_diameter_m[bin_idx])
    floc_diameter = max(floc_diameter, wavelength_m)
    ...
    # Scale aggregate diffraction to the raw weight of the removed monomer
    # diffraction component, so this changes angular shape, not event rate.
    diff_sum = np.sum(primary_diffraction)
    agg_sum = np.sum(aggregate_diffraction)
    ...
    if FLOC_INTERNAL_TRANSPORT_ENABLED:
        transported_internal, transport_info = _floc_internal_transport_component(
            bin_idx,
            primary_internal,
            rep_diameter,
            wavelength_m,
        ...
        tau_transport = float(transport_info.get("tau_transport", 0.0))
        tau_transport = np.clip(
            tau_transport,
            FLOC_INTERNAL_TRANSPORT_MIN_TAU,
            FLOC_INTERNAL_TRANSPORT_MAX_TAU
        )

        f_transport = 1.0 - np.exp(-tau_transport)
        f_transport = np.clip(f_transport, 0.0, 1.0)

        internal_component = (
            (1.0 - f_transport) * coherent_internal +
            f_transport * transported_internal
        )

        transport_info["f_transport"] = f_transport
        transport_info["rg_um"] = rg * 1.0e6
        transport_info["n_synth"] = n_synth
        transport_info["n_physical"] = n_physical
        transport_info["rep_idx"] = rep_idx
        transport_info["rep_diameter_um"] = rep_diameter * 1.0e6
        transport_info["floc_diameter_um"] = floc_diameter * 1.0e6
        floc_internal_transport_diagnostics[int(bin_idx)] = transport_info
    else:
        internal_component = coherent_internal
