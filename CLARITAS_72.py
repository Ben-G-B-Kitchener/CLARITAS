...
mie_cache_version = "claritas_7p2_explicit_floc_exit_position_v1"
...
def trace_rays_gpu(angles_init_np, primary_roughness_std_rad, floc_roughness_std_rad,
    ...
    with h5py.File(hdf5_file, "w") as f:
        ...
        for start in tqdm(range(0, N, estimated_chunk), total=(N + estimated_chunk - 1)//estimated_chunk, desc="Tracing rays"):
            ...
            floc_domain_dx_chunk_dev = cp.full((sz,), 0.0, dtype=cp.float32)
            floc_domain_dy_chunk_dev = cp.full((sz,), 0.0, dtype=cp.float32)
            floc_domain_path_chunk_dev = cp.full((sz,), 0.0, dtype=cp.float32)
            floc_internal_scatter_count_chunk_dev = cp.full((sz,), 0, dtype=cp.int32)
                ...
                try:
                    trace_kernel((blocks,), (threads_per_block,),
                        (
                            ...
                            absorbed_chunk_dev,
                            floc_domain_dx_chunk_dev,
                            floc_domain_dy_chunk_dev,
                            floc_domain_path_chunk_dev,
                            floc_internal_scatter_count_chunk_dev,
                            np.uint32(seed0),
                            np.uint32(seed1),
                            np.uint32(seed2)
                        ))
            ...
            dset_floc_domain_dx[start:end] = cp.asnumpy(floc_domain_dx_chunk_dev)
            dset_floc_domain_dy[start:end] = cp.asnumpy(floc_domain_dy_chunk_dev)
            dset_floc_domain_path[start:end] = cp.asnumpy(floc_domain_path_chunk_dev)
            dset_floc_internal_scatter_count[start:end] = cp.asnumpy(floc_internal_scatter_count_chunk_dev)

            del angles_chunk, exit_dir_chunk_dev, exit_x_chunk_dev, exit_y_chunk_dev, rpl_chunk_dev, scatter_count_chunk_dev, floc_event_count_chunk_dev, last_event_was_floc_chunk_dev, last_scatter_bin_chunk_dev, extinction_count_chunk_dev, absorbed_chunk_dev, floc_domain_dx_chunk_dev, floc_domain_dy_chunk_dev, floc_domain_path_chunk_dev, floc_internal_scatter_count_chunk_dev
            cp._default_memory_pool.free_all_blocks()
...
for wl_idx, wl in enumerate(wavelengths):
    ...
    if n_valid_exits > 0:
        floc_displacement_m = np.sqrt(floc_domain_dx**2 + floc_domain_dy**2)
