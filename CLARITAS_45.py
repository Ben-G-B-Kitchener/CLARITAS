...
mie_cache_version = "claritas_4p5_pair_histogram_structure_factor_v1"
...
def _pair_distance_structure_factor(theta_rad_values, wavelength_m, points):
    """
    Histogram-based Debye pair-distance structure factor.

    The earlier implementation evaluated sin(q r)/(q r) using either every
    pair distance or a deterministic pair subsample.  That was memory-safe but
    still expensive because it repeatedly formed q-by-r working arrays.

    This version keeps the same process-based physics:

        S(q) = 1 + (2/N) sum_{i<j} sin(q r_ij)/(q r_ij)

    but compresses the pair distances into a deterministic histogram first:

        sum_{i<j} f(r_ij)  ->  sum_k count_k f(r_k)

    where r_k is the bin centre.  This is not a detector-fit parameter; it is a
    numerical quadrature of the same pair-distance distribution.  It makes the
    structure-factor calculation scalable and reproducible while preserving the
    aggregate geometry dependence.
    """
    ...
    # Pair distances. For the current capped synthetic aggregate sizes this is
    # small enough to compute exactly, and the expensive q dependence is handled
    # later using the compressed histogram.
    ...
    # Pair-distance histogram resolution.  This is a numerical quadrature
    # resolution, not a model/fitting parameter.  Use enough bins to preserve
    # the pair-distance distribution but avoid huge q-by-pair arrays.
    ...
    # Chunk q only.  The second dimension is now histogram bins rather than all
    # particle pairs, so this remains compact and deterministic.
    q_chunk = 2048
    for start in range(0, len(q), q_chunk):
        end = min(len(q), start + q_chunk)
        qr = q[start:end, None] * r_centres[None, :]

        sinc = np.ones_like(qr, dtype=np.float64)
        nz = np.abs(qr) > 1.0e-12
        sinc[nz] = np.sin(qr[nz]) / qr[nz]

        pair_sum = np.sum(sinc * hist_counts[None, :], axis=1)
...
# ================= FLOCCULATED AGGREGATE PHASE-FUNCTION DIAGNOSTICS =================
# CLARITAS_45b: diagnostic only. No transport or physics changes.
#
# Purpose:
#   Verify that the currently cached/generated floc angular CDFs are genuinely
#   producing the intended aggregate phase functions before the GPU transport
#   uses them. This is deliberately placed immediately after angular table
#   generation/loading, so it diagnoses exactly the same cdf_profiles object
#   passed to the CUDA kernel later.

if np.any(particle_is_floc):
    diagnostic_angles_deg = np.array([
        0.0, 1.0, 5.0, 10.0, 20.0, 30.0,
        60.0, 90.0, 120.0, 130.0, 140.0,
        150.0, 160.0, 170.0, 180.0
    ], dtype=np.float64)

    diagnostic_theta_idx = np.array([
        closest_index(theta_deg, angle)
    ...
    cdf_table_first_wl = np.asarray(cdf_profiles[0], dtype=np.float64)

    for bin_idx in np.where(particle_is_floc)[0]:
        cdf_this = np.asarray(cdf_table_first_wl[bin_idx], dtype=np.float64)

        # Convert CDF back to the probability mass actually sampled by CUDA.
        # This is the angular law after any cache/load step and after all
        # profile normalisation used to form the CDF.
        phase_prob = np.diff(np.concatenate([[0.0], cdf_this]))
        phase_prob = np.maximum(phase_prob, 0.0)

        phase_sum = np.sum(phase_prob)
        if phase_sum > 0.0:
            phase_prob /= phase_sum
        else:
            phase_prob[:] = 1.0 / len(phase_prob)

        phase_peak = np.max(phase_prob)
        if phase_peak > 0.0:
            phase_peaknorm = phase_prob / phase_peak
        else:
            phase_peaknorm = phase_prob.copy()

        floc_band = int(floc_band_index_by_effective_bin[bin_idx])
        effective_diameter_um = particle_diameter_m[bin_idx] * 1.0e6
        source_min_um = source_primary_min_diameter_m[bin_idx] * 1.0e6
        source_max_um = source_primary_max_diameter_m[bin_idx] * 1.0e6
        source_mid_um = (
            np.sqrt(
                max(source_primary_min_diameter_m[bin_idx], 0.0) *
                max(source_primary_max_diameter_m[bin_idx], 0.0)
            ) * 1.0e6
        )

        i0 = phase_peaknorm[closest_index(theta_deg, 0.0)]
        i10 = phase_peaknorm[closest_index(theta_deg, 10.0)]
        i90 = phase_peaknorm[closest_index(theta_deg, 90.0)]
        i120 = phase_peaknorm[closest_index(theta_deg, 120.0)]
        i150 = phase_peaknorm[closest_index(theta_deg, 150.0)]
        i170 = phase_peaknorm[closest_index(theta_deg, 170.0)]

        row = {
            "effective_bin_index": int(bin_idx),
            "floc_band_index": floc_band,
            "effective_diameter_um": effective_diameter_um,
            "source_primary_min_um": source_min_um,
            "source_primary_max_um": source_max_um,
            "source_primary_mid_um": source_mid_um,
            ...
            "fraction_of_all_floc_events": float(
                particle_event_weights[bin_idx] / floc_event_probability
                if floc_event_probability > 0.0 else 0.0
            ),
            "phase_checksum_sum": float(np.sum(phase_prob)),
            "phase_peak_probability": float(phase_peak),
            "phase_entropy": float(-np.sum(
                phase_prob[phase_prob > 0.0] *
                np.log(phase_prob[phase_prob > 0.0])
            )),
            "I10_over_I0_peaknorm": float(i10 / i0 if i0 > 0.0 else np.nan),
            "I90_over_I0_peaknorm": float(i90 / i0 if i0 > 0.0 else np.nan),
            "I120_over_I0_peaknorm": float(i120 / i0 if i0 > 0.0 else np.nan),
            "I150_over_I0_peaknorm": float(i150 / i0 if i0 > 0.0 else np.nan),
            "I170_over_I0_peaknorm": float(i170 / i0 if i0 > 0.0 else np.nan),
        }

        for angle, tidx in zip(diagnostic_angles_deg, diagnostic_theta_idx):
            angle_label = f"{angle:.0f}" if abs(angle - round(angle)) < 1e-9 else f"{angle:g}"
            row[f"I_peaknorm_{angle_label}deg"] = float(phase_peaknorm[tidx])
            row[f"phase_prob_{angle_label}deg"] = float(phase_prob[tidx])
            row[f"CDF_{angle_label}deg"] = float(cdf_this[tidx])

        floc_phase_rows.append(row)

        floc_phase_profiles.append(pd.DataFrame({
            "Angle_deg": theta_deg,
            "effective_bin_index": int(bin_idx),
            "floc_band_index": floc_band,
            "effective_diameter_um": effective_diameter_um,
            "phase_probability": phase_prob,
            "phase_peaknorm": phase_peaknorm,
            "cdf": cdf_this,
        }))

    floc_phase_diag_df = pd.DataFrame(floc_phase_rows)
    floc_phase_diag_path = os.path.join(
        OUTDIR,
        f"floc_phase_function_diagnostics_conc_{mass_concentration_g_per_L}.csv"
    )
    floc_phase_diag_df.to_csv(floc_phase_diag_path, index=False)
    print(f"✅ Saved {floc_phase_diag_path}")

    floc_phase_profiles_df = pd.concat(
        floc_phase_profiles,
        ignore_index=True
    )
    floc_phase_profiles_path = os.path.join(
        OUTDIR,
        f"floc_phase_function_profiles_conc_{mass_concentration_g_per_L}.csv"
    )
    floc_phase_profiles_df.to_csv(floc_phase_profiles_path, index=False)
    print(f"✅ Saved {floc_phase_profiles_path}")

    # Compact visual check: plot peak-normalised floc phase functions.
    plt.figure(figsize=(9, 5))
    for bin_idx in np.where(particle_is_floc)[0]:
        prof = floc_phase_profiles_df[
            floc_phase_profiles_df["effective_bin_index"] == int(bin_idx)
        ]
        plt.plot(
            prof["Angle_deg"],
            prof["phase_peaknorm"],
            label=f"{particle_diameter_m[bin_idx]*1.0e6:.0f} um"
        )

    plt.xlabel("Scattering angle (deg)")
    plt.ylabel("Peak-normalised phase probability")
    plt.title("Floc phase functions actually sampled by CUDA")
    plt.yscale("log")
    plt.grid(True, alpha=0.3)
    plt.legend(title="Floc diameter", fontsize=8)
    plt.tight_layout()
    floc_phase_plot_path = os.path.join(
        OUTDIR,
        f"floc_phase_function_profiles_conc_{mass_concentration_g_per_L}.png"
    )
    plt.savefig(floc_phase_plot_path, dpi=200)
    plt.close()
    print(f"✅ Saved {floc_phase_plot_path}")

else:
    print("No floc bins present; skipped floc phase-function diagnostics.")
