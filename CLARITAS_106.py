# CLARITAS_105: Polarised vector radiative transfer Monte Carlo
...
#
# Physics (v105 improvements over v104):
...
#   - Material-specific floc size bins (loess: 5-250µm, kaolin: 0.5-50µm)
#   - NEW: Grain shielding — depth-dependent photon weighting (exp(-μ_t × depth))
#   - Photons scattering deep in shadowed volume are downweighted
...
FLOC_POOL_EFFECTIVE_DIAMETER_M_KAOLIN = np.array([
    2.0e-6, 4.0e-6, 6.0e-6, 8.0e-6, 12.0e-6, 20.0e-6,
    35.0e-6, 50.0e-6, 70.0e-6, 100.0e-6, 150.0e-6, 200.0e-6
], dtype=np.float64)
...
# === Grain shielding (v105) ==================================================

GRAIN_SHIELDING_ENABLED = False  # disabled in v106; requires kernel-level depth tracking
...
if _CUPY_AVAILABLE:
    ...
    def trace_rays_gpu(angles_init_np, R, ray_off, beam_sigma, collimated,
        ...
        with h5py.File(hdf5_file, "w") as f:
            f.attrs["claritas_version"] = "105_gpu"
...
print("="*70)
print("CLARITAS_105: Polarised Vector Radiative Transfer Monte Carlo (v105: grain shielding)")
...
for wl_idx, wl in enumerate(wavelengths):
    ...
    # --- Grain shielding weighting (v105) ---
    if GRAIN_SHIELDING_ENABLED and n_valid > 0:
        # Approximate depth traversed through illuminated volume:
        # depth_from_entrance ≈ exit_y + R_effective (y is upward, entry at -R)
        depth_from_entrance = exit_y[valid_mask] + R_effective
        depth_from_entrance = np.maximum(depth_from_entrance, 0.0)
        # Weight: exp(-mu_t * depth) — photons that traversed deeper
        # are attenuated by shielding and contribute less
        raw_weights = np.exp(-mu_t * depth_from_entrance)
        # Normalize so total weight = number of photons (preserves statistics)
        photon_weights = raw_weights / np.mean(raw_weights)
        mean_depth = np.mean(depth_from_entrance)
        print(f"  Grain shielding: mean depth={mean_depth:.3e}m, "
              f"attenuation range=[{np.min(raw_weights):.3f}, {np.max(raw_weights):.3f}]")
    else:
        photon_weights = np.ones(n_valid)
    ...
    # Use weighted counting
    w_idx = 0
    for vi, dj in zip(valid_idx, det_assigned):
        if dj >= 0:
            w = float(photon_weights[w_idx])
            det_counts[dj] += w
            det_stokes[dj]["I"].append(sI[vi] * w)
            det_stokes[dj]["Q"].append(sQ[vi] * w)
            det_stokes[dj]["U"].append(sU[vi] * w)
            det_stokes[dj]["V"].append(sV[vi] * w)
        w_idx += 1
...
plt.title("Detector Response — CLARITAS_105 Polarised MC")
...
if len(wavelengths)==1:
    ...
    plt.suptitle(f"Polarisation Diagnostics — CLARITAS_105 — {wl_nm} nm",fontsize=14,fontweight="bold")
    plt.tight_layout()
...
for wl_idx,wl in enumerate(wavelengths):
    ...
    if os.path.exists(hf):
        ...
        plt.title(f"Scatter Count Histogram — CLARITAS_105 — {wl_nm} nm")
