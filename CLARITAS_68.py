#!/usr/bin/env python3
# CLARITAS_68_true_floc_domain_spatial_exit.py
...
# CLARITAS_61 population experiment:
#   Uses CLARITAS_18-style source bands, 40-120 um floc diameter range,
#   shorter collision length, and narrow kernel pooling while retaining
#   CLARITAS_60 transport/optics.
...
FLOC_POOL_KERNEL_LOG_SIGMA = 0.25
...
ANGULAR_CDF_POWER = 0.0  # CLARITAS_65: planar/raw angular CDF for 2D radial detector geometry
...
# ================= FLOC MINI-DOMAIN TRANSPORT (CLARITAS_63) =================
# A floc scattering event is now converted into a precomputed angular CDF by
# tracing rays through a small 2D porous-floc domain.  This is still used to
# build the per-bin angle_cdf_table, so the CUDA kernel remains unchanged, but
# the floc angular law now represents multiple internal monomer scatters before
# the photon exits the aggregate.
...
# CLARITAS_64: true per-floc-event internal random walk in CUDA.
# The precomputed floc CDF remains available as a fallback/diagnostic, but when
# this is enabled the CUDA kernel samples multiple representative-monomer
# scatters inside a floc event before continuing through the bulk cell.
FLOC_CUDA_TRUE_INTERNAL_DOMAIN_ENABLED = True

# CLARITAS_68: true internal floc-domain transport now changes both exit
# direction and exit position.  A floc event is treated as entry through the
# near surface of a finite circular floc domain, followed by internal random
# walk until the ray exits the far surface.  The CUDA kernel then displaces
# the bulk ray by the internal entry-to-exit offset instead of treating the
# floc as a point scatterer.
...
mie_cache_version = "claritas_6p8_true_cuda_floc_domain_spatial_exit_v1"
...
def _floc_internal_domain_transport_profile(
    ...
    """
    CLARITAS_63 miniature floc-domain transport.

    A photon enters a circular floc cross-section from the incident direction,
    scatters from representative monomers using the internal monomer angular
    CDF, and exits the floc.  The returned profile is the distribution of exit
    direction relative to the incident direction.

    This is deliberately precomputed on the CPU as part of the angular-table
    build.  The main CUDA transport kernel remains unchanged: a floc event
    still samples one angle, but that angle is now the net exit angle from a
    miniature internal random walk rather than a single monomer-like deflection.
    """
    P_internal = _normalise_profile_for_cdf(primary_internal_component)
    internal_sum = float(np.sum(P_internal))
    ...
    if mu_s_inside_floc_per_m <= 0.0:
        # No internal scattering: all internally transported weight exits forward.
    ...
    # Angular CDF for the internal representative-monomer scatter.  This uses
    # the same angular measure helper as the main transport CDF, so switching
    # ANGULAR_CDF_POWER remains globally consistent.
    ...
    for _ in range(n_rays):
        # 2D projected encounter through a circular aggregate cross-section.
        # y is uniform across the projected diameter, matching the 2D detector
        # transport geometry used by the outer CUDA kernel.
        y = rng.uniform(-floc_radius_m, floc_radius_m)
        x = -np.sqrt(max(floc_radius_m*floc_radius_m - y*y, 0.0)) + eps
        ...
        for _step in range(max_scatters):
            ...
            # Solve intersection from current point along current direction
            # with x^2 + y^2 = R^2.
            b = x * vx + y * vy
            c = x*x + y*y - floc_radius_m*floc_radius_m
            disc = b*b - c
            ...
            ray_angle = np.arctan2(vy, vx) + theta
            vx = float(np.cos(ray_angle))
            vy = float(np.sin(ray_angle))
        ...
        # Net projected exit angle relative to the incident +x direction.
        # The outer kernel will randomly choose +/- this deflection, so store
        # the absolute polar deflection 0..pi.
    ...
    # Convert sparse Monte Carlo exits into a smooth angular intensity on the
    # full theta grid. Preserve the raw angular weight of P_internal.
...
def floc_synthetic_structure_profile(bin_idx, wavelength_m, mu_values, theta_rad_values):
    """
    CLARITAS_63 floc phase function with miniature internal floc-domain transport.

    The main CUDA kernel still samples one net angle per floc event, but that
    net angle is now precomputed by transporting rays through a small porous
    floc domain.  This implements the missing step repeatedly discussed earlier:

        enter floc -> multiple internal monomer scatters -> exit floc

    The PSD, cross-sections, event weighting, albedo, detector geometry and CUDA
    ...
    # Aggregate-scale diffraction from the whole floc.  This keeps the narrow
    # coherent forward component attached to the aggregate size rather than the
    # representative monomer size.
...
# ================= CLARITAS_64 CUDA INTERNAL-FLOC DOMAIN TABLES =================
def build_cuda_internal_floc_domain_tables():
    """
    Build per-bin tables used by the CUDA kernel for true internal floc-domain
    transport.

    For primary bins, the internal CDF is just the normal transport CDF and
    tau_s is zero.

    For floc bins, the internal CDF is the representative solid-primary Mie CDF,
    and tau_s is the raw internal scattering optical depth already derived from
    the porous-floc monomer density and representative monomer sigma_s.

    This lets the CUDA kernel do:
        floc event -> N internal monomer scatters -> continue bulk transport
    instead of compressing the floc into one pre-shaped phase-function sample.
    """
    ...
    for wl_idx, wl in enumerate(wavelengths):
        mu = np.cos(theta_rad)
        per_bin_internal_cdf = []
        per_bin_tau_s = []

        for bin_idx, is_floc in enumerate(particle_is_floc):
            if is_floc:
                n_synth, n_physical, rep_idx = _representative_monomer_count_for_floc_bin(bin_idx)
                rep_diameter = primary_particle_diameter_m[rep_idx]
                rep_radius = rep_diameter / 2.0
                ...
                _, internal_cdf = build_angular_pdf_and_cdf(I_rep, theta_rad)

                # Reuse the existing process-derived internal optical-depth code.
                # Only tau_s is used by CUDA for the number of raw internal monomer
                # scatter attempts.  This is deliberately not the reduced tau_transport.
                _, tau_s, tau_transport, transport_fraction = _floc_internal_transport_component(
                    bin_idx,
                ...
                per_bin_tau_s.append(float(tau_s))
            else:
                internal_cdf = np.asarray(cdf_profiles[wl_idx][bin_idx], dtype=np.float64)
                per_bin_tau_s.append(0.0)

            per_bin_internal_cdf.append(np.asarray(internal_cdf, dtype=np.float64))

        internal_cdf_profiles.append(np.asarray(per_bin_internal_cdf, dtype=np.float64))
        floc_internal_tau_s_profiles.append(np.asarray(per_bin_tau_s, dtype=np.float64))

    return internal_cdf_profiles, floc_internal_tau_s_profiles


internal_floc_angle_cdf_profiles, floc_internal_tau_s_profiles = build_cuda_internal_floc_domain_tables()
print("✅ Built CLARITAS_64 CUDA true internal-floc domain tables")
if np.any(particle_is_floc):
    tau0 = floc_internal_tau_s_profiles[0][particle_is_floc]
    print(
        "CLARITAS_64 floc raw internal tau_s range: "
    ...
    unsigned int seed2)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (is_floc_event && FLOC_CUDA_TRUE_INTERNAL_DOMAIN_ENABLED) {
            // CLARITAS_68 true internal floc-domain transport with spatial
            // entry-to-exit displacement.  The outer extinction event is treated
            // as an encounter with a finite circular floc domain.  The ray enters
            ...
            if (tau_s > 0.0f && floc_diameter > 0.0f) {
                ...
                // Start at the upstream surface of a circular floc whose centre
                ...
                while (internal_scatter_count < FLOC_INTERNAL_DOMAIN_MAX_SCATTERS) {
                    float u_step = fmaxf(rnd_uniform(&state), 1.0e-12f);
                    float step_m = -logf(u_step) * internal_mfp;

                    float nx = lx + vx * step_m;
                    float ny = ly + vy * step_m;

                    if (nx * nx + ny * ny >= rf * rf) {
                        // The sampled free path leaves the circular floc before
                        // another internal scatter.  Move exactly to the exit
                        // surface along the current direction.
                        float bq = 2.0f * (lx * vx + ly * vy);
                        float cq = lx * lx + ly * ly - rf * rf;
                    ...
                    float u_angle_internal = rnd_uniform(&state);
                    int idx_internal = cdf_binary_search(
                        &internal_angle_cdf_table[angle_offset],
                        n_theta,
                        u_angle_internal
                    );

                    float theta_internal = (float)theta_table[idx_internal];
                    float sign_internal =
                        (rnd_uniform(&stateJITTER) < 0.5f) ? -1.0f : 1.0f;

                    float ray_angle_internal = atan2f(vy, vx);
                    float new_angle_internal =
                        ray_angle_internal + sign_internal * theta_internal;

                    vx = cosf(new_angle_internal);
                    vy = sinf(new_angle_internal);
                ...
                float new_angle = ray_angle + sign2 * theta_3d;
                vx = cosf(new_angle);
                vy = sinf(new_angle);
            }
            ...
            if (roughness_jitter != 0.0f) {
                float ray_angle = atan2f(vy, vx);
                float new_angle = ray_angle + roughness_jitter;
                vx = cosf(new_angle);
                vy = sinf(new_angle);
            }
                ...
                try:
                    trace_kernel((blocks,), (threads_per_block,),
                        (
                            ...
                            np.int32(1 if FLOC_CUDA_TRUE_INTERNAL_DOMAIN_ENABLED else 0),
...
def save_detector_response_by_scatter_class(
    ...
    """
    CLARITAS_67 diagnostic only. No transport or optical physics is changed.

    Splits the radial detector response into scatter-count classes so we can
    see which transport regime is responsible for each part of the detector
    profile:

        ballistic      scatter_count == 0
        single         scatter_count == 1
        low_multiple   scatter_count 2..5
        high_multiple  scatter_count > 5

    The detector-hit rule matches the production detector binning used by the
    existing diagnostics: ballistic rays are excluded from rear-detector bins
    with centre >= 90 degrees.
    """
    exit_x = np.asarray(exit_x)
    exit_y = np.asarray(exit_y)
    scatter_count = np.asarray(scatter_count)
    ...
    # First pass: raw counts by detector and class.
    for centre in detector_angles_deg:
        delta = circular_angle_difference_deg(exit_position_angle_deg, centre)
        hit_mask_raw = valid_exit_mask & (np.abs(delta) <= detector_acceptance_deg)

        # Match existing production detector-hit rule.
        if centre >= 90:
            hit_mask_base = hit_mask_raw & (scatter_count > 0)
        else:
            hit_mask_base = hit_mask_raw
    ...
    # show where each transport class sends its light, independent of how much
    # total light that class contributes.
    ...
    # Detector-normalised total profile for direct comparison with the usual
    # CLARITAS output normalisation.
    detector_totals = (
