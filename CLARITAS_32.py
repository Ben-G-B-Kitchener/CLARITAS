...
#     aggregate structure-factor CDF: primary-material form factor × S(q)
#
...
# Legacy HG aggregate anisotropy retained for comparison/debug only.
# The RDG-FA floc angular kernel below does not use FLOC_G.
FLOC_G = -0.8
...
# Floc g values are not represented by a single HG g in this RDG-FA variant,
...
print(f"FLOC_G legacy HG anisotropy (not used by RDG-FA floc kernel): {FLOC_G:.6f}")
    ...
    for wl in wavelengths:
        ...
        for bin_idx, (radius, weight, refr_index, is_floc) in enumerate(zip(
            ...
            if is_floc:
                ...
                # representative primary-material form factor × fractal
    ...
    unsigned int seed2)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            // CDFs generated from representative primary form factor × structure factor.
