...
# Process-based ray tracer with banded target-diameter floc PSD and banded floc density.
...
# ============================ FLOCCULATION MODEL ============================
# Flocculation is handled as a banded fixed effective PSD transformation.
#
# No concentration-growth function is used here.
#
# Method:
#   - Start from the selected sediment PSD, e.g. kaolin or loess.
#   - The floc band table defines which primary-size bins become flocs.
#   - Bins above the final floc band remain individual particles.
#   - Floc bins receive a banded target effective diameter.
#   - Only those floc bins receive banded lower floc effective density.
#   - Non-floc bins keep the original particle diameter and density.
#
...
FLOC_ENABLED = True
# Banded floc enlargement model.
#
# This avoids an artificial smooth growth law that can saturate all floc bins at
# the same multiplier. Instead, each primary-size band is mapped to a plausible
# target effective floc diameter. The multiplier is then:
#
#   multiplier = target_floc_diameter / primary_diameter
#
# only for bins that fall within the floc band table.
#
# Literature-consistent behaviour:
#   - clay/fine silt forms compact microflocs
#   - larger silt near the cutoff is incorporated less strongly
#   - grains above the cutoff remain individual particles
#
# Edit these bands directly if microscopy/PSD measurements suggest different
# effective floc sizes.
FLOC_BAND_PRIMARY_MAX_DIAMETER_M = np.array([
    0.5e-6,
    1.0e-6,
    2.0e-6,
    5.0e-6,
...
FLOC_BAND_TARGET_DIAMETER_M = np.array([
    30.0e-6,
    40.0e-6,
    50.0e-6,
    60.0e-6,
...
# Safety clamp on the implied diameter multiplier.
FLOC_DIAMETER_MULTIPLIER_MIN = 1.0
FLOC_DIAMETER_MULTIPLIER_MAX = 150.0

# Banded floc effective density model.
#
# This avoids another smooth power-law tuning function. Each effective floc-size
# band is assigned a plausible literature-style effective density.
#
# D is the effective floc diameter after the banded diameter transform.
#
# Typical interpretation:
#   compact small microflocs   -> higher density
#   larger porous microflocs   -> lower density
#   fragile macroflocs         -> lowest density
#
# Edit these bands directly if settling/microscopy measurements suggest
# different density ranges.
FLOC_DENSITY_BAND_MAX_DIAMETER_M = np.array([
    20.0e-6,
    50.0e-6,
    100.0e-6,
    200.0e-6,
...
FLOC_DENSITY_BAND_KG_PER_M3 = np.array([
    1000.0,
    700.0,
    400.0,
    200.0,
...
# Safety clamp on assigned floc effective density.
...
PRIMARY_REFLECT_SIZE_THRESHOLD = 50.0e-6

FLOC_REFLECT_PROB = 0.07
...
particle_density_kg_per_m3 = 2000.0  # loess density
...
# Per-bin floc multiplier. Non-floc bins remain at multiplier 1.0.
...
if FLOC_ENABLED:
    particle_is_floc = primary_particle_diameter_m <= FLOC_BAND_PRIMARY_MAX_DIAMETER_M[-1]

    # Map each flocculating primary diameter to a target effective floc diameter.
    target_floc_diameter_by_bin = primary_particle_diameter_m.copy()

    for band_max_diameter, target_floc_diameter in zip(
        FLOC_BAND_PRIMARY_MAX_DIAMETER_M,
        FLOC_BAND_TARGET_DIAMETER_M
    ):
        band_mask = (
            particle_is_floc &
            (primary_particle_diameter_m <= band_max_diameter) &
            (target_floc_diameter_by_bin == primary_particle_diameter_m)
        )

        target_floc_diameter_by_bin[band_mask] = target_floc_diameter

    # Any flocculating bins inside the final band but not otherwise assigned
    # are assigned the final target diameter.
    unassigned_floc_mask = (
        particle_is_floc &
        (target_floc_diameter_by_bin == primary_particle_diameter_m)
    )

    if np.any(unassigned_floc_mask):
        target_floc_diameter_by_bin[unassigned_floc_mask] = (
            FLOC_BAND_TARGET_DIAMETER_M[-1]
        )

    raw_floc_multiplier = np.ones_like(primary_particle_diameter_m, dtype=np.float64)
    raw_floc_multiplier[particle_is_floc] = (
        target_floc_diameter_by_bin[particle_is_floc] /
        primary_particle_diameter_m[particle_is_floc]
    )

    floc_diameter_multiplier_by_bin[particle_is_floc] = np.clip(
        raw_floc_multiplier[particle_is_floc],
        FLOC_DIAMETER_MULTIPLIER_MIN,
        FLOC_DIAMETER_MULTIPLIER_MAX
    )

    # Banded effective floc PSD transformation.
    particle_diameter_m[particle_is_floc] = (
        primary_particle_diameter_m[particle_is_floc] *
        floc_diameter_multiplier_by_bin[particle_is_floc]
    )

    # Banded floc effective density. Only floc bins are changed.
    floc_effective_density_by_bin_kg_per_m3 = np.full_like(
        primary_particle_diameter_m,
        particle_density_kg_per_m3,
        dtype=np.float64
    )

    target_floc_density_by_bin = np.full_like(
        primary_particle_diameter_m,
        particle_density_kg_per_m3,
        dtype=np.float64
    )

    for band_max_diameter, band_density in zip(
        FLOC_DENSITY_BAND_MAX_DIAMETER_M,
        FLOC_DENSITY_BAND_KG_PER_M3
    ):
        band_mask = (
            particle_is_floc &
            (particle_diameter_m <= band_max_diameter) &
            (target_floc_density_by_bin == particle_density_kg_per_m3)
        )

        target_floc_density_by_bin[band_mask] = band_density

    # Any floc bins above the final density band receive the last density value.
    unassigned_density_mask = (
        particle_is_floc &
        (target_floc_density_by_bin == particle_density_kg_per_m3)
    )

    if np.any(unassigned_density_mask):
        target_floc_density_by_bin[unassigned_density_mask] = (
            FLOC_DENSITY_BAND_KG_PER_M3[-1]
        )

    floc_effective_density_by_bin_kg_per_m3[particle_is_floc] = np.clip(
        target_floc_density_by_bin[particle_is_floc],
        FLOC_DENSITY_MIN_KG_PER_M3,
        FLOC_DENSITY_MAX_KG_PER_M3
    )

    particle_density_by_bin_kg_per_m3[particle_is_floc] = (
        floc_effective_density_by_bin_kg_per_m3[particle_is_floc]
    )
else:
    particle_is_floc = np.zeros_like(primary_particle_diameter_m, dtype=bool)
    floc_effective_density_by_bin_kg_per_m3 = np.full_like(
        primary_particle_diameter_m,
        particle_density_kg_per_m3,
        dtype=np.float64
...
# Legacy raw PSD CDF retained only for diagnostics/backwards compatibility.
...
print(f"FLOC_DIAMETER_MULTIPLIER_MIN: {FLOC_DIAMETER_MULTIPLIER_MIN:.3f}")
print(f"FLOC_DIAMETER_MULTIPLIER_MAX: {FLOC_DIAMETER_MULTIPLIER_MAX:.3f}")
print("FLOC_BAND_PRIMARY_MAX_DIAMETER_um:", np.round(FLOC_BAND_PRIMARY_MAX_DIAMETER_M * 1e6, 3).tolist())
print("FLOC_BAND_TARGET_DIAMETER_um:", np.round(FLOC_BAND_TARGET_DIAMETER_M * 1e6, 3).tolist())
print(f"effective_floc_cutoff_from_final_band_um: {FLOC_BAND_PRIMARY_MAX_DIAMETER_M[-1]*1e6:.3f}")
...
print("FLOC_DENSITY_BAND_MAX_DIAMETER_um:", np.round(FLOC_DENSITY_BAND_MAX_DIAMETER_M * 1e6, 3).tolist())
print("FLOC_DENSITY_BAND_KG_PER_M3:", np.round(FLOC_DENSITY_BAND_KG_PER_M3, 3).tolist())
...
print(f"flocculating_bins: {np.sum(particle_is_floc)} / {len(particle_is_floc)}")
...
print(f"primary_floc_diameter_range_um: {np.min(primary_particle_diameter_m[particle_is_floc])*1e6 if np.any(particle_is_floc) else 0.0:.3f} - {np.max(primary_particle_diameter_m[particle_is_floc])*1e6 if np.any(particle_is_floc) else 0.0:.3f}")
...
mie_cache_version = "per_particle_intensity_no_sin_theta_banded_floc_and_density_no_max_d_v1"
...
mie_cache_key = (
    ...
    str(FLOC_BAND_PRIMARY_MAX_DIAMETER_M.tolist()) +
    str(FLOC_BAND_TARGET_DIAMETER_M.tolist()) +
    str(floc_diameter_multiplier_by_bin.tolist()) +
    str(FLOC_DENSITY_BAND_MAX_DIAMETER_M.tolist()) +
    str(FLOC_DENSITY_BAND_KG_PER_M3.tolist()) +
...
df_psd_compare = pd.DataFrame({
    "primary_diameter_um": primary_um,
    "effective_diameter_um": effective_um,
    "target_floc_diameter_um": particle_diameter_m * 1e6,
...
unique_densities = np.unique(density_values)
...
plt.bar(
    ...
    alpha=0.35,
    color="tab:blue",
    label=f"Original PSD ({particle_density_kg_per_m3:.0f} kg/m³)"
)

# -------------------------
# Effective PSD - unchanged bins
# -------------------------
...
plt.bar(
    ...
    alpha=0.85,
    color="tab:blue",
    label=f"Effective PSD (unchanged bins)"
)

# -------------------------
# Effective PSD - floc bins
# -------------------------
...
plt.bar(
    ...
    alpha=0.85,
    color="tab:red",
    label="Effective PSD (floc bins, size-dependent density)"
)

plt.xscale("log")
plt.xlabel("Particle / floc diameter (µm)")
plt.ylabel("PSD weight fraction")
plt.title("Original PSD vs Effective PSD")
plt.grid(True, which="both", alpha=0.3)
plt.legend()
plt.tight_layout()
...
floc_diag_png_1 = os.path.join(
    OUTDIR,
    "floc_primary_vs_effective_diameter.png"
)
...
floc_diag_png_3 = os.path.join(
    OUTDIR,
    "floc_multiplier_vs_primary_diameter.png"
)
...
# -------------------------
# 1. Primary diameter vs effective diameter
# -------------------------

plt.figure(figsize=(8, 6))

plt.scatter(
    primary_um[~particle_is_floc],
...
plt.xscale("log")
plt.yscale("log")
plt.xlabel("Primary diameter (µm)")
plt.ylabel("Effective diameter (µm)")
plt.title("Primary Diameter vs Effective Floc Diameter")
plt.grid(True, which="both", alpha=0.3)
plt.legend()
plt.tight_layout()
...
# -------------------------
# 2. Effective floc density vs effective diameter
# -------------------------
...
# -------------------------
# 3. Floc multiplier vs primary diameter
# -------------------------
...
plt.xscale("log")
plt.xlabel("Primary diameter (µm)")
plt.ylabel("Diameter multiplier")
plt.title("Floc Diameter Multiplier vs Primary Diameter")
plt.grid(True, which="both", alpha=0.3)
plt.legend()
plt.tight_layout()
...
plt.title("Effective Refractive Index vs Floc Diameter")
plt.grid(True, which="both", alpha=0.3)
plt.legend()
plt.tight_layout()
