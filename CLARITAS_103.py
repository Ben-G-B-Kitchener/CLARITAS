# CLARITAS_103: Polarised vector radiative transfer Monte Carlo
...
#
# Physics (v103 improvements over v102):
...
#   - NEW: Flocculation RE-ENABLED with all v102 physics active
#   - NEW: Conc-dependent Df and dependent scattering active for flocs
...
if _CUPY_AVAILABLE:
    ...
    def trace_rays_gpu(angles_init_np, R, ray_off, beam_sigma, collimated,
        ...
        with h5py.File(hdf5_file, "w") as f:
            f.attrs["claritas_version"] = "103_gpu"
...
print("="*70)
print("CLARITAS_103: Polarised Vector Radiative Transfer Monte Carlo (v103: flocs + v102 physics)")
...
plt.title("Detector Response — CLARITAS_103 Polarised MC")
...
if len(wavelengths)==1:
    ...
    plt.suptitle(f"Polarisation Diagnostics — CLARITAS_103 — {wl_nm} nm",fontsize=14,fontweight="bold")
    plt.tight_layout()
...
for wl_idx,wl in enumerate(wavelengths):
    ...
    if os.path.exists(hf):
        ...
        plt.title(f"Scatter Count Histogram — CLARITAS_103 — {wl_nm} nm")
