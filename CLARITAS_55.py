...
FLOC_POOL_KERNEL_LOG_SIGMA = 0.85
...
if FLOC_ENABLED:
    ...
    for source_band_idx in range(n_source_bands):
        ...
        if np.sum(row) <= 0.0:
            # Defensive fallback to the nearest floc bin if an over-aggressive
            # threshold removes every contribution.
            nearest = int(np.argmin(np.abs(log_ratio)))
            row[nearest] = 1.0
...
mie_cache_version = "claritas_5p5_kernel_floc_pooling_v1"
...
for wl_idx, wl in enumerate(wavelengths):
    ...
    save_floc_bin_detector_proxy_diagnostics(
        int(wl * 1e9),
        angle_cdf_table_np,
        theta_table_np,
        detector_angles,
...
if FLOC_ENABLED and len(FLOC_POOL_PRIMARY_BAND_MAX_DIAMETER_M) > 0:
    floc_band_summary_df = pd.DataFrame({
        ...
        "floc_pooling_kernel": floc_pooling_kernel,
        "floc_mass_by_source_band_and_floc_bin_kg": floc_mass_by_source_band_and_floc_bin_kg,
        "floc_effective_density_by_source_band_and_floc_bin_kg_per_m3": floc_effective_density_by_source_band_and_floc_bin_kg_per_m3,
