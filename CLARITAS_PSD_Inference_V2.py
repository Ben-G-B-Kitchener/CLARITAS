#!/usr/bin/env python3
"""CLARITAS effective-PSD inference V2.

This is an inverse-model wrapper around the unchanged V23.2 3-D geometric /
Snell / Fresnel forward physics in ``claritas_forward_inference_v2.py``.

V2 deliberately removes the main structural restriction in V1.  Instead of a
...
from claritas_forward_inference_v2 import ClaritasForwardModel, get_material_psd
...
class PSDInferenceV2:
    def __init__(self, material: str, measurements: pd.DataFrame, config: dict,
        ...
        if self.concentrations.size < 2:
            raise ValueError("V2 expects at least two measured concentrations")
    ...
    def load_initial_parameters(self, path: str | None) -> np.ndarray:
        ...
        if p.size != self.n_params:
            raise ValueError(f"Initial parameter file has {p.size} parameters; V2 expects {self.n_params}")
        print(f"Loaded initial parameters from {path}")
        return p

    def load_v1_warm_start(self, path: str) -> np.ndarray:
        """Approximate V1 effective PSDs with the V2 seven-knot representation."""
        df = pd.read_csv(path)
        ...
        for j, c in enumerate(self.concentrations):
            ...
            # Sample the V1 log-ratio at the V2 knot locations, then remove the
        ...
        print(f"Warm-started V2 PSD controls from V1 effective PSDs: {path}")
        return p

    def optimise(self, initial_parameters: str | None = None, skip_local: bool = False,
                 v1_warm_start: str | None = None):
        print(f"\n=== {self.material.upper()} PSD inference V2 ===")
        ...
        print("V23.2 optical physics is fixed. Only suspension-state PSD and local concentration are inferred.\n")
    ...
    def write_outputs(self, opt_result, p_best: np.ndarray, final_score: float,
        ...
        for x in conditions:
            ...
            for a, mv, mod, h in zip(res.detector_angles_deg, measured,
                                     res.normalized_response, res.raw_hits):
        ...
        plt.title(f"{self.material.capitalize()} V2 inferred optical PSD modification")
        ...
        plt.title(f"{self.material.capitalize()} source and V2 effective PSDs")
        plt.grid(True, which="both", alpha=0.25); plt.legend(); plt.tight_layout()
        ...
        for x in conditions:
            ...
            plt.plot(x["result"].detector_angles_deg, x["result"].normalized_response, "--",
                     label=f"V2 fit {c:g} g/L")
        plt.xlabel("Detector angle (deg)"); plt.ylabel("Normalized detector response")
        plt.title(f"{self.material.capitalize()} measured vs V2 inferred-suspension CLARITAS response")
        ...
        plt.title(f"{self.material.capitalize()} V2 detector residuals")
...
def main():
    ap = argparse.ArgumentParser(
        description="CLARITAS PSD Inference V2: concentration-specific non-monotonic PSDs + effective local concentration")
    ...
    ap.add_argument("--config", default="psd_inference_v2_config.json")
    ap.add_argument("--output-dir", default="psd_inference_v2_results")
    ...
    ap.add_argument("--initial-parameters", help="V2 best_fit_parameters.json/checkpoint to resume or refine")
    ap.add_argument("--warm-start-v1", help="V1 effective_psds.csv used to initialise the seven-knot V2 PSDs")
    ...
    for mat in materials:
        inf = PSDInferenceV2(mat, df, cfg, root / mat, quick=args.quick)
        if args.baseline_only:
            score = inf.baseline_only()
            print(f"{mat} V2 baseline objective: {score:.8g}")
        else:
            inf.optimise(initial_parameters=args.initial_parameters, skip_local=args.skip_local,
