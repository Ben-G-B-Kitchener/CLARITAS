#!/usr/bin/env python3
# CLARITAS_21_30-08-2026_geometric_refraction.py
#
# Pure geometric-optics particle transport fork of CLARITAS.
#
# PHYSICS MODEL
# -------------
...
#   - source beam, circular sample geometry, CUDA transport/chunking, heatmaps,
...
#         delta = 2*(i-r)
#   - the ray/sphere plane has random azimuth and is projected into the existing
#     2-D CLARITAS transport plane
#
...
# Fresnel reflection and absorption are also omitted in this first pure-geometric
...
# point uniform over projected disk (rho=sqrt(U)), random ray/sphere-plane azimuth,
    ...
    unsigned int seed1)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL + 1.0e-8f) {
        ...
        // coefficient.  For MU_GEOM == 0 the free path is infinite.
...
def trace_rays_gpu(
    ...
    sampled directly from the exponential free-path distribution defined by mu_geom.
    The HDF5 ``interaction_count`` dataset records the number of geometric sphere
    encounters experienced by each ray.
    """
    ...
    with h5py.File(hdf5_file, "w") as f:
        ...
        dset_interaction_count = f.create_dataset("interaction_count", shape=(N,), dtype='i4', chunks=(estimated_chunk,))
        ...
        for start in tqdm(
            ...
            interaction_count_chunk_dev = cp.full((sz,), -1, dtype=cp.int32)
            ...
            dset_interaction_count[start:end] = cp.asnumpy(interaction_count_chunk_dev)
...
#        norm=LogNorm(vmin=1, vmax=np.max(heatmap)),
    ...
    # 0..180 degree detector/deflection-angle scale.
    ...
    interacted_valid = (interaction_count[detector_valid] > 0)
...
# Summary of the pure geometric-refraction transport model.
...
print("Particle diameter affects encounter frequency and event-bin selection through n_i*pi*r_i^2.")
...
print("Transparent-limit check: as mass_concentration_g_per_L -> 0, mu_geom -> 0 and particle encounter probability across the sample -> 0.")
print("=======================================================")
