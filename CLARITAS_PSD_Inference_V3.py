#!/usr/bin/env python3
"""CLARITAS effective-PSD inference V3 for the TARDIIS apparatus model.

This is an inverse-model wrapper around the CLARITAS V24 3-D geometric / Snell / Fresnel particle physics plus the
...
from claritas_tardiis_core_v24 import TardiisForwardModel as ClaritasForwardModel, get_material_psd
...
class PSDInferenceV3:
    ...
    def optimise(self, initial_parameters: str | None = None, skip_local: bool = False,
        ...
        print(f"\n=== {self.material.upper()} PSD inference V3 / TARDIIS apparatus ===")
        ...
        print("V24 TARDIIS apparatus + particle optical physics is fixed. Only suspension-state PSD and local concentration are inferred.\n")
    ...
    def write_outputs(self, opt_result, p_best: np.ndarray, final_score: float,
        ...
        plt.title(f"{self.material.capitalize()} V3 inferred optical PSD modification")
        ...
        plt.title(f"{self.material.capitalize()} source and V3 effective PSDs")
        plt.grid(True, which="both", alpha=0.25); plt.legend(); plt.tight_layout()
        ...
        for x in conditions:
            ...
            plt.plot(x["result"].detector_angles_deg, x["result"].normalized_response, "--",
                     label=f"V3 fit {c:g} g/L")
        plt.xlabel("Detector angle (deg)"); plt.ylabel("Normalized detector response")
        plt.title(f"{self.material.capitalize()} measured vs V3 TARDIIS-apparatus inferred-suspension response")
        ...
        plt.title(f"{self.material.capitalize()} V3 detector residuals")
...
def main():
    ap = argparse.ArgumentParser(
        description="CLARITAS PSD Inference V3: V24 TARDIIS apparatus + non-monotonic effective PSD/local concentration inference")
    ap.add_argument("--material", choices=["kaolin", "loess", "all"], default="all")
    ap.add_argument("--measurements", default="measured_detector_responses_v3.csv")
    ...
    for mat in materials:
        inf = PSDInferenceV3(mat, df, cfg, root / mat, quick=args.quick)
        if args.baseline_only:
            score = inf.baseline_only()
            print(f"{mat} V3 baseline objective: {score:.8g}")
        else:
