# CLARITAS_78: production CUDA transport with validated 3-D direction handling,
...
for idx, (r, refr_index, is_floc) in enumerate(zip(
    ...
    if is_floc:
        # Floc cross-section: use effective-medium Mie or fall back to
        # geometric scatter efficiency.  The effective-medium index was
        # precomputed by Maxwell Garnett and stored in
        # particle_complex_refractive_index_by_bin.
        floc_m_rel = particle_complex_refractive_index_by_bin[idx] / n_medium
        floc_x = 2.0 * np.pi * n_medium * r / scatter_probability_wavelength

        qext, qsca, qback, g_floc = miepython.efficiencies_mx(
            floc_m_rel,
            floc_x
        )
        ...
        g_by_bin.append(float(np.real(g_floc)))

        # Retain the mean-chord absorption diagnostics for consistency
        # with the old budget reports, even though absorption is now
        # determined by Mie qabs.
        k_eff = (
...
AGGREGATE_COHERENCE_LIMIT_ENABLED = False
...
def build_or_load_cuda_internal_floc_domain_tables():
    ...
    if os.path.exists(internal_cache_file):
        ...
        floc_internal_albedo_profiles = np.asarray(
            cache.get("floc_internal_albedo_profiles", np.ones_like(floc_internal_tau_s_profiles)),
            dtype=np.float64
        )
        return internal_cdf_profiles, [
            ...
            np.asarray(row, dtype=np.float64)
            for row in floc_internal_albedo_profiles
        ]
    ...
    for wl_idx, wl in enumerate(wavelengths):
        ...
        per_bin_albedo = np.ones(len(particle_is_floc), dtype=np.float64)
        ...
        for bin_idx in floc_bins:
            ...
            # Representative-monomer single-scattering albedo for internal
            # absorption checks inside the floc domain walk.
            rep_radius_m = rep_diameter / 2.0
            rep_m_rel = solid_primary_complex_index / n_medium
            rep_x = 2.0 * np.pi * n_medium * rep_radius_m / wl
            qext_rep, qsca_rep, qback_rep, g_rep = miepython.efficiencies_mx(rep_m_rel, rep_x)
            qext_val = max(float(np.real(qext_rep)), 0.0)
            qsca_val = min(max(float(np.real(qsca_rep)), 0.0), qext_val)
            per_bin_albedo[bin_idx] = float(qsca_val / qext_val if qext_val > 0.0 else 1.0)

        floc_internal_tau_s_profiles.append(per_bin_tau_s)
        floc_internal_albedo_profiles.append(per_bin_albedo)
    ...
    return internal_cdf_profiles, floc_internal_tau_s_profiles, floc_internal_albedo_profiles


internal_floc_angle_cdf_profiles, floc_internal_tau_s_profiles, floc_internal_albedo_profiles = (
...
__device__ float fresnel_transmission_probability(
    float n_incident,
    float n_exit,
    float cos_theta_i)
{
    // cos_theta_i = |n dot v| where n points from surface into the medium
    float cos_i = fabsf(cos_theta_i);
    cos_i = fminf(cos_i, 1.0f);
    float sin_i = sqrtf(fmaxf(1.0f - cos_i * cos_i, 0.0f));
    float sin_t = (n_incident / n_exit) * sin_i;

    // Total internal reflection
    if (sin_t >= 1.0f) return 0.0f;

    float cos_t = sqrtf(fmaxf(1.0f - sin_t * sin_t, 0.0f));

    // Fresnel equations for unpolarised light
    float rs = (n_incident * cos_i - n_exit * cos_t)
             / (n_incident * cos_i + n_exit * cos_t);
    float rp = (n_exit * cos_i - n_incident * cos_t)
             / (n_exit * cos_i + n_incident * cos_t);

    float R = 0.5f * (rs * rs + rp * rp);
    return 1.0f - R;  // transmission probability
}

__device__ void refract_direction(
    float n_incident,
    float n_exit,
    float vx_in, float vy_in, float vz_in,
    float nx_surf, float ny_surf, float nz_surf,
    float* vx_out, float* vy_out, float* vz_out)
{
    // Snell's law for direction only (surface normal points OUT of medium)
    float cos_i = -(vx_in * nx_surf + vy_in * ny_surf + vz_in * nz_surf);
    cos_i = fminf(fmaxf(cos_i, -1.0f), 1.0f);

    float ratio = n_incident / n_exit;
    float sin2_t = ratio * ratio * (1.0f - cos_i * cos_i);

    if (sin2_t > 1.0f) {
        // Total internal reflection - reflect around surface normal
        float dot_n = vx_in * nx_surf + vy_in * ny_surf + vz_in * nz_surf;
        *vx_out = vx_in - 2.0f * dot_n * nx_surf;
        *vy_out = vy_in - 2.0f * dot_n * ny_surf;
        *vz_out = vz_in - 2.0f * dot_n * nz_surf;
        return;
    }

    float cos_t = sqrtf(fmaxf(1.0f - sin2_t, 0.0f));
    float mu = ratio * cos_i - cos_t;

    *vx_out = ratio * vx_in + mu * nx_surf;
    *vy_out = ratio * vy_in + mu * ny_surf;
    *vz_out = ratio * vz_in + mu * nz_surf;

    // Normalise
    float norm = sqrtf(fmaxf(
...
__global__ void trace_kernel(
    ...
    const double* floc_internal_albedo_table,
    ...
    const int BOUNDARY_FRESNEL_ENABLED,
    ...
    const unsigned long long ray_index_offset)
{
    ...
    while (x * x + y * y + z * z <= R_REAL * R_REAL) {
        ...
        if (exited) {
            // CLARITAS_78: boundary Fresnel refraction/reflection.
            // The ray has reached (or passed) the sphere boundary.  Before
            // registering an exit, apply the dielectric interface physics.
            if (BOUNDARY_FRESNEL_ENABLED && N_MEDIUM != N_EXTERNAL) {
                // Outward-pointing surface normal (boundary is a sphere)
                float r_dist = sqrtf(fmaxf(x*x + y*y + z*z, 1.0e-30f));
                float nx = x / r_dist;
                float ny = y / r_dist;
                float nz = z / r_dist;

                // Cosine of incident angle inside medium (v pointing toward surface)
                float cos_i = vx * nx + vy * ny + vz * nz;
                // If cos_i is negative, the ray is pointing inward (shouldn't happen at a genuine exit)
                cos_i = fmaxf(cos_i, 0.0f);

                float T = fresnel_transmission_probability(
                    N_MEDIUM, N_EXTERNAL, cos_i
                );

                if (rnd_uniform_double(&state) < (double)T) {
                    // Ray transmits through the boundary.  Refract the exit
                    // direction and proceed with the exit record.
                    float vx_refracted, vy_refracted, vz_refracted;
                    refract_direction(
                        N_MEDIUM, N_EXTERNAL,
                        vx, vy, vz,
                        nx, ny, nz,
                        &vx_refracted, &vy_refracted, &vz_refracted
                    );
                    vx = vx_refracted;
                    vy = vy_refracted;
                    vz = vz_refracted;
                    // The exit position is already at the boundary; break
                    // normally to record the exit.
                }
                else {
                    // Ray reflects internally.  Reflect direction around
                    // the surface normal and continue propagation.
                    float dot_n = vx * nx + vy * ny + vz * nz;
                    vx = vx - 2.0f * dot_n * nx;
                    vy = vy - 2.0f * dot_n * ny;
                    vz = vz - 2.0f * dot_n * nz;

                    // Renormalise to suppress numerical drift
                    float vnorm = sqrtf(fmaxf(vx*vx + vy*vy + vz*vz, 1.0e-30f));
                    vx /= vnorm;
                    vy /= vnorm;
                    vz /= vnorm;

                    // Continue the propagation loop rather than exiting.
                    continue;
                }
            }
            break;
        ...
        if (is_floc_event && FLOC_CUDA_TRUE_INTERNAL_DOMAIN_ENABLED) {
            ...
            if (tau_s > 0.0f && floc_diameter > 0.0f) {
                ...
                while (internal_scatter_count < FLOC_INTERNAL_DOMAIN_MAX_SCATTERS) {
                    ...
                    // CLARITAS_78: internal floc absorption.
                    // After each scatter, evaluate the representative-monomer
                    // single-scattering albedo.  If the photon is absorbed
                    // inside the floc, terminate the outer ray.
                    double floc_alb = floc_internal_albedo_table[pidx];
                    floc_alb = fmin(1.0, fmax(0.0, floc_alb));
                    if (rnd_uniform_double(&state) >= floc_alb) {
                        absorbed_internal = 1;
                        break;
                    }
                }

                if (absorbed_internal) {
                    absorbed = 1;
                    break;
                }
...
def trace_rays_gpu(angles_init_np, primary_roughness_std_rad, floc_roughness_std_rad,
    ...
    floc_internal_albedo_dev = cp.asarray(floc_internal_albedo_profiles[0], dtype=cp.float64)
                ...
                try:
                    ...
                    trace_kernel((blocks,), (threads_per_block,),
                        (
                            ...
                            np.int32(1 if BOUNDARY_FRESNEL_ENABLED else 0),
                            np.float32(n_medium),
