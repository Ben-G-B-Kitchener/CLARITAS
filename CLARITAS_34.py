...
#   - floc angular scattering is sampled from a Mie-structure-factor aggregate CDF
#
...
# Mie-structure-factor angular CDF, not forward/side/back fitted ratios.
...
#   angular scattering is sampled from a Mie-structure-factor CDF
#
# Therefore floc event rate is no longer Mie-derived.
# g_by_bin for flocs is diagnostic only here; the CUDA kernel samples the
# floc CDF directly rather than using a scalar anisotropy.
sigma_s = []
g_by_bin = []
...
for r, refr_index, is_floc in zip(
    ...
    if is_floc:
        ...
        # Placeholder diagnostic; floc angular behaviour comes from the
        # Mie-structure aggregate CDF built below.
        g_by_bin.append(0.0)
    else:
...
#
# Floc bins:
#   - Mie-structure-factor aggregate angular profile
...
mie_cache_version = "claritas_3p1_primary_mie_floc_mie_structure_blend_v1"
...
def _fractal_structure_factor(theta_rad_values, wavelength_m, floc_diameter_m, df):
    """
    Smooth fractal aggregate structure factor shape for CDF construction.

    This is used as an angular-shape factor, not as an absolute cross-section.
    It is peak-normalised before blending so that event rate remains controlled
    by sigma_floc = FLOC_SCATTER_EFFICIENCY*pi*r_floc^2.
    """
    df = float(np.clip(df, 1.1, 3.0))
    q = (4.0 * np.pi * n_medium / wavelength_m) * np.sin(theta_rad_values / 2.0)

    # Approximate radius of gyration from outer radius. For a compact sphere
    # Rg = sqrt(3/5)*R; this is used here as a scale, not an extra fit parameter.
    r_outer = max(float(floc_diameter_m) / 2.0, 1.0e-12)
    rg = np.sqrt(3.0 / 5.0) * r_outer

    # Teixeira-like smooth fractal roll-off. The exact prefactor is less
    # important than preserving the physically expected q^(-Df) high-q trend.
    qr2 = (q * rg) ** 2
    S = 1.0 / (1.0 + qr2 / max(df, 1.0e-12)) ** (df / 2.0)
    S = _normalise_profile_for_cdf(S)
    S /= np.max(S) if np.max(S) > 0.0 else 1.0
    return S


def _floc_optical_depth_beta(bin_idx, wavelength_m):
    """
    Optical-depth blend for a floc bin.

    beta = 1 - exp(-tau)

    tau is estimated from the number of representative source-primary monomers
    in the aggregate times the representative monomer Mie scattering cross-section,
    divided by the projected floc area. This uses existing PSD/floc quantities.
    """
    ...
    n_monomer_equiv = max(float(n_monomer_equiv), 1.0)

    m_rel = n_particle / n_medium
    x_mie = 2.0 * np.pi * n_medium * r_rep / wavelength_m
    _qext, qsca, _qback, _g = miepython.efficiencies_mx(m_rel, x_mie)
    sigma_rep = max(float(qsca), 0.0) * np.pi * r_rep**2

    floc_area = np.pi * (particle_diameter_m[bin_idx] / 2.0) ** 2
    tau = n_monomer_equiv * sigma_rep / floc_area if floc_area > 0.0 else 0.0
    tau = max(float(tau), 0.0)
    beta = 1.0 - np.exp(-tau)
    beta = float(np.clip(beta, 0.0, 1.0))
    return beta, tau, n_monomer_equiv, rep_idx


def floc_mie_structure_blend_profile(bin_idx, wavelength_m, mu_values, theta_rad_values):
    """
    Mie-structure-factor floc phase function.

    I_floc(theta) = (1-beta) * Mie_rep(theta) * S(theta) + beta * S(theta)

    beta is derived from aggregate optical thickness, so optically thin flocs
    retain more representative source-primary form-factor character, while
    optically thicker flocs move toward structure-dominated aggregate scattering.
    """
    beta, tau, n_monomer_equiv, rep_idx = _floc_optical_depth_beta(
        bin_idx,
        wavelength_m
    )

    r_rep = primary_particle_diameter_m[rep_idx] / 2.0
    m_rel = n_particle / n_medium
    x = 2.0 * np.pi * n_medium * r_rep / wavelength_m
    S1, S2 = miepython.S1_S2(m_rel, x, mu_values)
    mie_I = 0.5 * (np.abs(S1)**2 + np.abs(S2)**2)
    mie_I = _normalise_profile_for_cdf(mie_I)
    mie_I /= np.max(mie_I) if np.max(mie_I) > 0.0 else 1.0

    S_struct = _fractal_structure_factor(
        theta_rad_values,
        wavelength_m,
        particle_diameter_m[bin_idx],
        FLOC_FRACTAL_DIMENSION
    )

    I = (1.0 - beta) * mie_I * S_struct + beta * S_struct
...
if os.path.exists(mie_cache_file):
    ...
    print("Building angular tables: primary Mie + floc Mie-structure aggregate diagnostics...")
    ...
    for wl in wavelengths:
        ...
        for bin_idx, (radius, weight, refr_index, is_floc) in enumerate(zip(
            ...
            if is_floc:
                # Flocs use a Mie-structure-factor aggregate phase function.
                # The representative source-primary Mie form factor is blended
                # with a fractal structure factor according to the floc optical
                # thickness. No scalar phase-function sampler is used.
                I = floc_mie_structure_blend_profile(
                    bin_idx,
                    wl,
                    mu,
                    theta_rad
            ...
            if is_floc:
                print(
                    "RDG/FLOC DEBUG:",
                    "bin", bin_idx,
                    "diam_um", particle_diameter_m[bin_idx] * 1e6,
                    "I0", I[closest_index(theta_deg, 0.0)],
                    "I90", I[closest_index(theta_deg, 90.0)],
                    "I170", I[closest_index(theta_deg, 170.0)],
                    "I170/I0", I[closest_index(theta_deg, 170.0)] / max(I[closest_index(theta_deg, 0.0)], 1e-300)
                )
            # Direct angular CDF.
...
plt.title("Angular scattering profiles - primary Mie + floc Mie-structure aggregate")
    ...
    unsigned int seed2)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            // Sample the per-bin angular CDF.
            // Primary bins contain Mie CDFs.
            // Floc bins contain Mie-structure-factor aggregate CDFs.
