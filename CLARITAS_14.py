...
# Process-based ray tracer with detailed diameter- and density-dependent floc PSD and dual reflection controls.
...
mass_concentration_g_per_L = 2.0
scatter_prob_per_step = 1.0

# ============================ FLOCCULATION MODEL ============================
# Flocculation is handled as a fixed effective PSD transformation.
#
# No concentration-growth function is used here.
#
# Method:
#   - Start from the selected sediment PSD, e.g. kaolin or loess.
#   - Bins with primary diameter <= FLOC_PRIMARY_MAX_DIAMETER are treated as flocs.
#   - Those bins receive a diameter-dependent larger effective diameter.
#   - Only those floc bins receive size-dependent lower floc effective density.
...
FLOC_ENABLED = True
FLOC_PRIMARY_MAX_DIAMETER = 15.0e-6

# Diameter-dependent floc enlargement.
#
# Smaller primary particles are allowed to form proportionally larger flocs,
# while larger grains near the flocculation cutoff are enlarged much less.
#
# Formula, applied only to bins where:
#   primary_particle_diameter_m <= FLOC_PRIMARY_MAX_DIAMETER
#
#   multiplier(d) =
#       FLOC_DIAMETER_MULTIPLIER_MAX *
#       (d / FLOC_REFERENCE_DIAMETER) ** (-FLOC_DIAMETER_MULTIPLIER_EXPONENT)
#
# then clipped between:
#   FLOC_DIAMETER_MULTIPLIER_MIN and FLOC_DIAMETER_MULTIPLIER_MAX
#
# This keeps flocculation as a fixed PSD transformation, not a concentration
# growth function.
FLOC_DIAMETER_MULTIPLIER_MIN = 1.0
FLOC_DIAMETER_MULTIPLIER_MAX = 50.0
FLOC_REFERENCE_DIAMETER = 1.0e-6
FLOC_DIAMETER_MULTIPLIER_EXPONENT = 0.01

# Size-dependent floc effective density.
# Literature generally supports decreasing effective density with increasing
# floc size. D is the effective floc diameter, not the primary diameter:
#
#   rho_floc(D) = FLOC_DENSITY_REF_KG_PER_M3 *
#                 (D / FLOC_DENSITY_REF_DIAMETER) ** (-FLOC_DENSITY_EXPONENT)
#
# clipped between FLOC_DENSITY_MIN_KG_PER_M3 and FLOC_DENSITY_MAX_KG_PER_M3.
FLOC_DENSITY_REF_KG_PER_M3 = 300.0
FLOC_DENSITY_REF_DIAMETER = 10.0e-6
FLOC_DENSITY_EXPONENT = 0.5
...
if FLOC_ENABLED:
    particle_is_floc = primary_particle_diameter_m <= FLOC_PRIMARY_MAX_DIAMETER

    raw_floc_multiplier = (
        FLOC_DIAMETER_MULTIPLIER_MAX *
        (primary_particle_diameter_m / FLOC_REFERENCE_DIAMETER)
        ** (-FLOC_DIAMETER_MULTIPLIER_EXPONENT)
    )
    ...
    # Diameter-dependent effective floc PSD transformation.
    ...
    # Size-dependent floc effective density. Only floc bins are changed.
    raw_floc_density_by_bin = (
        FLOC_DENSITY_REF_KG_PER_M3 *
        (particle_diameter_m / FLOC_DENSITY_REF_DIAMETER)
        ** (-FLOC_DENSITY_EXPONENT)
    )
    ...
    floc_effective_density_by_bin_kg_per_m3[particle_is_floc] = np.clip(
        raw_floc_density_by_bin[particle_is_floc],
...
print(f"FLOC_PRIMARY_MAX_DIAMETER_um: {FLOC_PRIMARY_MAX_DIAMETER*1e6:.3f}")
...
print(f"FLOC_REFERENCE_DIAMETER_um: {FLOC_REFERENCE_DIAMETER*1e6:.3f}")
print(f"FLOC_DIAMETER_MULTIPLIER_EXPONENT: {FLOC_DIAMETER_MULTIPLIER_EXPONENT:.3f}")
...
print(f"FLOC_DENSITY_REF_KG_PER_M3: {FLOC_DENSITY_REF_KG_PER_M3:.3e}")
print(f"FLOC_DENSITY_REF_DIAMETER_um: {FLOC_DENSITY_REF_DIAMETER*1e6:.3f}")
print(f"FLOC_DENSITY_EXPONENT: {FLOC_DENSITY_EXPONENT:.3f}")
...
mie_cache_version = "per_particle_intensity_no_sin_theta_detailed_floc_model_v1"
...
mie_cache_key = (
    ...
    str(FLOC_ENABLED) +
    str(FLOC_PRIMARY_MAX_DIAMETER) +
    str(FLOC_DIAMETER_MULTIPLIER_MIN) +
    str(FLOC_DIAMETER_MULTIPLIER_MAX) +
    str(FLOC_REFERENCE_DIAMETER) +
    str(FLOC_DIAMETER_MULTIPLIER_EXPONENT) +
    str(floc_diameter_multiplier_by_bin.tolist()) +
    str(FLOC_DENSITY_REF_KG_PER_M3) +
    str(FLOC_DENSITY_REF_DIAMETER) +
    str(FLOC_DENSITY_EXPONENT) +
