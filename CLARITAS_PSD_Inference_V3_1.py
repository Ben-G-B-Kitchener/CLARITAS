#!/usr/bin/env python3
"""CLARITAS effective-PSD inference V3.1 using V24.1 variance-reduced TARDIIS detector scoring.

This is an inverse-model wrapper around the CLARITAS V24.1 3-D geometric / Snell / Fresnel particle physics plus the
physical TARDIIS cylindrical water/acrylic/air cell and detector collimators.
The shared forward implementation is ``claritas_tardiis_core_v24_1.py``.
...
from claritas_tardiis_core_v24_1 import TardiisForwardModel as ClaritasForwardModel, get_material_psd
...
class PSDInferenceV31:
    ...
    def optimise(self, initial_parameters: str | None = None, skip_local: bool = False,
        ...
        print(f"\n=== {self.material.upper()} PSD inference V3.1 / TARDIIS apparatus + VR detector scoring ===")
        ...
        print("V24.1 TARDIIS apparatus + particle optical physics is fixed; detector scoring is variance-reduced but non-fitted. Only suspension-state PSD and local concentration are inferred.\n")
    ...
    def write_outputs(self, opt_result, p_best: np.ndarray, final_score: float,
        ...
        for x in conditions:
            ...
            for a, mv, mod, h, vr, sup, exn in zip(
                    res.detector_angles_deg, measured, res.normalized_response, res.raw_hits,
                    res.variance_reduced_physical_equivalent_scores,
                    res.variance_reduced_support_counts,
                    res.normalized_exact_response):
                detector_rows.append({
                    ...
                    "modelled_normalized_exact": exn,
                    "raw_model_hits": int(h),
                    "variance_reduced_physical_equivalent_score": float(vr),
                    "variance_reduced_support_rays": int(sup),
                    "residual": mod - mv,
                })
            ...
            summary_rows.append({
                ...
                "detector_vr_equivalent_fraction": res.detector_vr_equivalent_fraction,
                "detector_scoring_mode": res.detector_scoring_mode,
                "detector_vr_bandwidth_factor": res.detector_vr_bandwidth_factor,
                "detector_vr_min_support": int(np.min(res.variance_reduced_support_counts)),
                "detector_vr_median_support": float(np.median(res.variance_reduced_support_counts)),
            })
        ...
        payload = {
            ...
            "detector_scoring": "V24.1 fixed-bandwidth 4-D product-Epanechnikov phase-space estimator; exact physical hits retained",
        ...
        plt.title(f"{self.material.capitalize()} V3.1 inferred optical PSD modification")
        ...
        plt.title(f"{self.material.capitalize()} source and V3.1 effective PSDs")
        plt.grid(True, which="both", alpha=0.25); plt.legend(); plt.tight_layout()
        ...
        for x in conditions:
            ...
            plt.plot(x["result"].detector_angles_deg, x["result"].normalized_response, "--",
                     label=f"V3.1 fit {c:g} g/L")
        plt.xlabel("Detector angle (deg)"); plt.ylabel("Normalized detector response")
        plt.title(f"{self.material.capitalize()} measured vs V3.1 TARDIIS-apparatus inferred-suspension response")
        ...
        plt.title(f"{self.material.capitalize()} V3.1 detector residuals")
...
def main():
    ap = argparse.ArgumentParser(
        description="CLARITAS PSD Inference V3.1: V24 TARDIIS apparatus + non-monotonic effective PSD/local concentration inference")
    ap.add_argument("--material", choices=["kaolin", "loess", "all"], default="all")
    ap.add_argument("--measurements", default="measured_detector_responses_v3_1.csv")
    ap.add_argument("--config", default="psd_inference_v3_1_config.json")
    ap.add_argument("--output-dir", default="psd_inference_v3_1_results")
    ap.add_argument("--quick", action="store_true", help="25k-ray / capped-evaluation exploratory fit")
    ...
    for mat in materials:
        inf = PSDInferenceV31(mat, df, cfg, root / mat, quick=args.quick)
        if args.baseline_only:
            score = inf.baseline_only()
            print(f"{mat} V3.1 baseline objective: {score:.8g}")
        else:
