...
# Physics, inputs, outputs, and public API preserved; geometric forced encounters, no reflection, fixed-step heatmap.
...
scat_prob_grad = 3.0e-8
### Reflection supresses wavelenth sensitivity ###
#REFLECT_PROB = 0.0 # probability of reflection at particle - TARDIIS & CLARITAS tuning
REFLECT_PROB = 0.1
...
#particle_density_kg_per_m3 = 300.0  # kaolin density with flocs
...
PSD_RESPONSE_BOOST = 1.0  # process-based 2D transport calibration; fixed multiplier on physical mu_s
SCATTER_2D_BOOST = PSD_RESPONSE_BOOST  # backward-compatible alias used by diagnostics/comments
...
# PSD cross-sectional-area loading correction.
# This makes scatter probability depend on samp_csa without making the boost
# itself concentration-dependent. Keep SAMP_CSA_REF fixed for a given instrument
# calibration; tune CSA_SCATTER_EXPONENT if needed.
#SAMP_CSA_REF = 6.1e-5
#CSA_SCATTER_EXPONENT = 0.7084
CSA_SCATTER_EXPONENT = 0.45
...
# PSD-weighted mean r^2, not including pi; retained as a diagnostic only.
...
# ------------------------------------------------------------------
# 3D Mie scattering coefficient retained as a diagnostic only.
#
# In the forced particle-encounter model, mu_s is NOT used to set
# STEP_SIZE or event probability. It is retained for diagnostics and
...
# ------------------------------------------------------------------
# PROCESS-BASED FORCED PARTICLE-ENCOUNTER MODEL
#
# Kernel interpretation:
#   scatter_prob_per_step = 1.0
#
# Therefore one kernel step represents one particle encounter.
#
# STEP_SIZE and particle selection must therefore be based on geometric
# encounter statistics, not on Qsca-weighted Mie scattering coefficients.
#
# For the 2D transport-plane representation, use a plane-section density:
#
#   N_A,i = n_i * h_i
#
# where:
#   n_i = 3D particle number density in bin i        [particles / m^3]
#   h_i = effective slice thickness for bin i        [m]
#
# Here h_i is a configurable fraction of particle diameter:
#
#   h_i = PLANE_SECTION_THICKNESS_SCALE * d_i
#
# Then:
#
#   N_A,total = sum_i(N_A,i)                         [particles / m^2]
#   STEP_SIZE = 1 / sqrt(N_A,total)                  [m]
#
# Particle event weights are geometric:
#
#   P(bin i | encounter) = N_A,i / sum_j(N_A,j)
#
# Mie still controls the emergent angular deflection after the encountered
# particle bin has been selected.
# ------------------------------------------------------------------

# Concentration-responsive effective transport-plane thickness.
#
# This keeps the model process-based:
#   - base particle population comes from concentration + PSD
#   - one kernel step is one particle encounter
#   - no optical scattering coefficient is used to set event spacing
#
# The exponent represents concentration-dependent crowding/overlap of the
# effective 2D transport plane. It reduces encounters at low concentration and
# increases encounters at high concentration relative to the reference case.
PLANE_SECTION_THICKNESS_SCALE_REF = 0.025
CONC_REF_G_PER_L = 1.5
PLANE_SECTION_CONC_EXPONENT = 0.0

PLANE_SECTION_THICKNESS_SCALE = (
    PLANE_SECTION_THICKNESS_SCALE_REF *
    (mass_concentration_g_per_L / CONC_REF_G_PER_L) **
    PLANE_SECTION_CONC_EXPONENT
)

effective_slice_thickness_by_bin = (
    PLANE_SECTION_THICKNESS_SCALE *
    particle_diameter_m
)

particle_plane_profile_density_by_bin = (
    particle_number_density_by_bin *
    effective_slice_thickness_by_bin
)

particle_plane_profile_density = np.sum(
    particle_plane_profile_density_by_bin
)

STEP_SIZE = (
    1.0 / np.sqrt(particle_plane_profile_density)
    if particle_plane_profile_density > 0.0
    else np.inf
)

# Forced-event kernel: every step is an encounter.
scatter_prob_per_step = 1.0 if particle_plane_profile_density > 0.0 else 0.0
scatter_prob_per_step = min(max(scatter_prob_per_step, 0.0), 1.0)

# Particle choice during a forced particle encounter is weighted by
# geometric encounter likelihood, not by Qsca-weighted mu_s contribution.
particle_event_weights = np.zeros_like(
    particle_plane_profile_density_by_bin,
    dtype=np.float64
)

if particle_plane_profile_density > 0:
    particle_event_weights = (
        particle_plane_profile_density_by_bin /
        particle_plane_profile_density
    )
...
# Diagnostic only: cross-section weighted by raw PSD fraction retained for comparison.
sigma_s_avg = np.sum(sigma_s * particle_weights)

# Geometric line-intercept loading, without Mie efficiency.
# Units: 1/m. Retained as a diagnostic only.
mu_geom = np.sum(
    particle_number_density_by_bin *
    np.pi *
    particle_radius_m**2
...
# Whole-sample diagnostic loadings.
# These are dimensionless and should NOT be used directly as per-step probability.
L_eff = 2.0 * R_REAL
samp_csa_geom = mu_geom * L_eff
samp_csa = mu_s * L_eff
samp_plane_profiles = particle_plane_profile_density * (2.0 * R_REAL)**2

print(f"PLANE_SECTION_THICKNESS_SCALE: {PLANE_SECTION_THICKNESS_SCALE:.3f}")
print(f"particle_plane_profile_density: {particle_plane_profile_density:.3e}")
print(f"geometric_STEP_SIZE: {STEP_SIZE:.3e}")

# Reflection path length disabled; stochastic reflection branch remains in the kernel but REFLECT_PROB is currently 0.0.
reflection_path_length = 0.0
...
print(f"samp_csa_3d_mie_diagnostic: {samp_csa:.3e}")
...
print(f"mu_s_3d_diagnostic: {mu_s:.3e}")
print(f"particle_plane_profile_density: {particle_plane_profile_density:.3e}")
print(f"plane_section_profile_spacing_m: {STEP_SIZE:.3e}")
print(f"mu_geom_line_intercept_diagnostic: {mu_geom:.3e}")
print(f"samp_plane_profiles_diagnostic: {samp_plane_profiles:.3e}")
#print(f"csa_scatter_gain: {csa_scatter_gain:.3e}")
#print(f"SAMP_CSA_REF: {SAMP_CSA_REF:.3e}")
print(f"PLANE_SECTION_THICKNESS_SCALE_REF: {PLANE_SECTION_THICKNESS_SCALE_REF:.3f}")
print(f"CONC_REF_G_PER_L: {CONC_REF_G_PER_L:.3f}")
print(f"PLANE_SECTION_CONC_EXPONENT: {PLANE_SECTION_CONC_EXPONENT:.3f}")
print(f"PLANE_SECTION_THICKNESS_SCALE: {PLANE_SECTION_THICKNESS_SCALE:.3f}")
print(f"CSA_SCATTER_EXPONENT: {CSA_SCATTER_EXPONENT:.4f}")
print(f"PSD_WEIGHT_MODE: {PSD_WEIGHT_MODE}")
print(f"PSD_RESPONSE_BOOST: {PSD_RESPONSE_BOOST:.4f}")
...
mie_cache_version = "per_particle_intensity_no_sin_theta_v3"
    ...
    unsigned int seed2)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            // Sample particle size from the interaction-rate CDF for this scattering event.
            ...
            // Particle roughness / non-sphericity broadening.
            // Zero-mean Gaussian angular perturbation, independent of concentration.
            ...
            // rpl is accumulated during propagation below.
            // Do not derive path length from step_count because encounter
            // distances are sampled stochastically.


                // Process-based particle reflection branch.
                // A fraction REFLECT_PROB of encountered particles reverse the ray
                // by approximately pi radians, optionally restricted by particle size.
...
//                if ((particle_diameter_table[pidx] >= RELFECT_SIZE_THRESHOLD) &&
//                    (rpl < REFLECTION_PATH_LENGTH)) {
//                    new_angle += sign2 * 3.1415927f;
//                }
        ...
        // Propagate ray
        // ----------------------------
        // Fixed visual/transport step:
        //   one loop is one particle encounter, and the ray is also drawn through
        //   the heatmap at the same spacing. This avoids sparse heatmaps caused by
        //   long exponential jumps. STEP_SIZE is still derived from the geometric
        //   forced-encounter model above.
        x += vx * STEP_SIZE;
        y += vy * STEP_SIZE;
        rpl += STEP_SIZE;
...
# ================= GPU wrapper function (adaptive chunking) =================
def trace_rays_gpu(angles_init_np, STEP_SIZE, scatter_prob, roughness_std_rad, reflection_path_length, n_medium, n_external, R_REAL, RAY_OFFSET, VIS_SIZE,
