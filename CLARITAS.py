...
mass_concentration_g_per_L = 0.0
...
RAY_OFFSET = 0.0001  # Ray initial y-offset (m)
#STEP_SIZE = 1.0e-6  # integration step size (m)
STEP_SIZE = 1.0e-9  # integration step size (m)
#VISUAL_SCALE = 100.0 TARDIIS
#VIS_SIZE = 2048      # Heatmap resolution TARDIIS
VISUAL_SCALE = 1.0
VIS_SIZE = 2048      # Heatmap resolution
N_RAYS = 100_000  # number of rays to simulate
MAX_ITERATIONS = int(1e30)  # maximum number of steps per ray to avoid infinite loops
...
# ================= MIE PROFILES (host side - unchanged) =================
theta_rad = np.deg2rad(theta_deg)
all_profiles = []
cdf_profiles = []

for wl in wavelengths:
    rnd_no = np.random.rand()
    cdf_idx = closest_index(cdf, rnd_no)

    k = 2*np.pi*n_medium / wl
    x = k * particle_radius_m[cdf_idx]
    m = n_particle / n_medium
    mu = np.cos(theta_rad)
    S1, S2 = miepython.S1_S2(m, x, mu)
    I = np.sqrt(S1**2) + np.sqrt(S2**2)
    I = np.real(I)
    all_profiles.append(I.astype(np.float64))
    cdf_profiles.append(np.cumsum(I).astype(np.float64))
    ...
    unsigned int seed2)
{
    ...
    // Gaussian beam waist
    // ----------------------------
    float beam_sigma = 0.00005f; // 0.05 mm beam radius sigma
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            for (int j = 0; j < cdf_n - 1; ++j) {
                if (u <= (float)cdf_table[j]) {
                    idx = j;
                    break;
                }
                if (j == cdf_n - 2) idx = cdf_n - 1;
            }

            float theta_new = (float)theta_table[idx];
            float sign = (rnd_uniform(&state) < 0.5f) ? -1.0f : 1.0f;
            float sign2 = (rnd_uniform(&stateJITTER) < 0.5f) ? -1.0f : 1.0f;

            float ray_angle = atan2f(vy, vx);
            float new_angle = ray_angle + sign * theta_new;
...
def trace_rays_gpu(angles_init_np, STEP_SIZE, scatter_prob, reflection_path_length, R_REAL, RAY_OFFSET, VIS_SIZE,
                   VISUAL_SCALE, cdf_table_np, theta_table_np,
    ...
    cdf_dev = cp.asarray(cdf_table_np, dtype=cp.float64)
    ...
    with h5py.File(hdf5_file, "w") as f:
        ...
        for start in tqdm(range(0, N, estimated_chunk), total=(N + estimated_chunk - 1)//estimated_chunk, desc="Tracing rays"):
            ...
            # Free GPU memory
            del angles_chunk, exit_dir_chunk_dev, exit_x_chunk_dev, exit_y_chunk_dev, rpl_chunk_dev
            cp._default_memory_pool.free_all_blocks()
...
for wl_idx, wl in enumerate(wavelengths):
    ...
    cdf_table_np = np.array(cdf_profiles[wl_idx], dtype=np.float64)
    cdf_table_np /= cdf_table_np[-1]  # normalize CDF
...
#        norm=LogNorm(vmin=1, vmax=np.max(heatmap)),
    ...
    pos_angles_semicircle = exit_pos_angles[(exit_pos_angles >= 0) & (exit_pos_angles <= 180)]
...
backscatter_fraction = backscatter / total
