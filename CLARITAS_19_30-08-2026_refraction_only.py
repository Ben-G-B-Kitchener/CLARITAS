#!/usr/bin/env python3
# CLARITAS_19_30-08-2026_refraction_only.py
#
...
#   - mu_s, STEP_SIZE, scatter_prob_per_step and n_i*sigma_i particle-event weighting
...
mass_concentration_g_per_L = 0.0000000001
...
if mu_s <= 0.0:
    raise ValueError("mu_s must be > 0 for the current transport stepping model")

# Preserved exactly from the supplied version.
STEP_SIZE = 0.1 / mu_s
...
print(f"STEP_SIZE: {STEP_SIZE:.3e} m")
print(f"Interaction probability per physical step: {scatter_prob_per_step:.3e}")
print("=====================================================\n")
    ...
    unsigned int seed1)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < interaction_prob) {
            interaction_count++;
...
def trace_rays_gpu(
    ...
    Per-ray outputs are streamed to HDF5. The legacy dataset name ``scatter_count``
    is retained for downstream compatibility, but it now means particle interaction
    count; every such interaction uses the refraction model above.
    """
...
for wl_idx, wl in enumerate(wavelengths):
    ...
    print(f"Mean interaction count: {np.mean(scatter_count):.3e}")
    print(f"Max interaction count: {np.max(scatter_count)}")
