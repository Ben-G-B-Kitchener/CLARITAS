# CLARITAS_101: Polarised vector radiative transfer Monte Carlo
...
#
# Physics (v101 improvements over v100):
...
#   - NEW: Concentration-dependent fractal dimension (Df increases with φ)
#   - NEW: Dependent scattering correction (packing factor) for dense media
#   - NEW: FLOC_SCATTER_EFFICIENCY now actually applied to cross-sections
#   - NEW: Floc-floc aggregation kernel (second collision mode)
...
FLOC_CONC_DEPENDENT_DF_ENABLED = False
...
DEPENDENT_SCATTERING_ENABLED = False
...
K_LOESS = 0.004   # iron-oxide-coated mineral dust
K_KAOLIN = 0.001  # pure kaolinite clay

# === Concentration-dependent sample geometry (v101) ==========================

CONC_DEPENDENT_RADIUS_ENABLED = True
RADIUS_REFERENCE_TAU = 1.5   # optical depth at reference (loess 0.5 g/L, R=0.049m)
...
# ---- Concentration-dependent effective radius (v101) -----------------------
if CONC_DEPENDENT_RADIUS_ENABLED:
    optical_depth = mu_t * R_REAL
    # Keep optical depth roughly constant: scale R so τ ≈ τ_ref
    R_effective = R_REAL * RADIUS_REFERENCE_TAU / max(optical_depth, RADIUS_REFERENCE_TAU)
    R_effective = max(RADIUS_MIN_M, min(R_effective, R_REAL))
    print(f"  Conc-dependent radius: R_eff={R_effective:.4f}m (τ={optical_depth:.2f}, τ_ref={RADIUS_REFERENCE_TAU})")
else:
    R_effective = R_REAL
...
if _CUPY_AVAILABLE:
    ...
    def trace_rays_gpu(angles_init_np, R, ray_off, beam_sigma, collimated,
        ...
        with h5py.File(hdf5_file, "w") as f:
            f.attrs["claritas_version"] = "101_gpu"
...
# Set material-specific k (v101)
if material_name == "loess":
    PRIMARY_REFRACTIVE_INDEX_IMAG_K = K_LOESS
elif material_name == "kaolin":
    PRIMARY_REFRACTIVE_INDEX_IMAG_K = K_KAOLIN
else:
    PRIMARY_REFRACTIVE_INDEX_IMAG_K = 0.004  # default

engine_label = "GPU (CuPy/CUDA)" if _CUPY_AVAILABLE else "CPU (multiprocessing)"

print("="*70)
print("CLARITAS_101: Polarised Vector Radiative Transfer Monte Carlo (v101: conc-dependent Df, dependent scattering)")
...
plt.title("Detector Response — CLARITAS_101 Polarised MC")
...
if len(wavelengths)==1:
    ...
    plt.suptitle(f"Polarisation Diagnostics — CLARITAS_101 — {wl_nm} nm",fontsize=14,fontweight="bold")
    plt.tight_layout()
...
for wl_idx,wl in enumerate(wavelengths):
    ...
    if os.path.exists(hf):
        ...
        plt.title(f"Scatter Count Histogram — CLARITAS_101 — {wl_nm} nm")
