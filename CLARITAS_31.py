...
# Runtime cap for unresolved internal interactions during one floc encounter.
# The per-bin expected count is derived from the fractal aggregate size multiplier.
FLOC_INTERNAL_MAX_SCATTERS = 16
...
# Internal-floc optical-depth / memory-loss proxy.
#
# A floc encounter is represented as several unresolved aggregate interactions.
# The expected number of internal interactions is derived from existing floc
# geometry, not fitted per band. For a fractal aggregate with
# N ~ (D_floc / D_primary)^Df, a projected random-walk / memory-loss scale is
# approximated as sqrt(N). For Df = 2 this is approximately the diameter
# multiplier.
floc_internal_tau_by_bin = np.zeros_like(
    particle_diameter_m,
    dtype=np.float64
)

if np.any(particle_is_floc):
    floc_internal_tau_by_bin[particle_is_floc] = (
        floc_diameter_multiplier_by_bin[particle_is_floc] **
        (FLOC_FRACTAL_DIMENSION / 2.0)
    )

    floc_internal_tau_by_bin[particle_is_floc] = np.clip(
        floc_internal_tau_by_bin[particle_is_floc],
        1.0,
        float(FLOC_INTERNAL_MAX_SCATTERS)
    )
...
print("Finite floc object displacement: enabled; residual chord length derived from selected floc diameter")
if np.any(particle_is_floc):
    print(
        f"floc_internal_tau_range: "
        f"{np.min(floc_internal_tau_by_bin[particle_is_floc]):.3f} - "
        f"{np.max(floc_internal_tau_by_bin[particle_is_floc]):.3f}"
    )
print(f"FLOC_INTERNAL_MAX_SCATTERS: {FLOC_INTERNAL_MAX_SCATTERS}")
...
mie_cache_version = "claritas_3p0_primary_mie_floc_finite_object_memory_loss_v1"
...
mie_cache_key = (
    ...
    str(FLOC_G) +
    str(FLOC_INTERNAL_MAX_SCATTERS) +
    str(floc_internal_tau_by_bin.tolist()) +
...
__global__ void trace_kernel(
    ...
    const double* floc_internal_tau_table,
    const double* angle_cdf_table,
    const double* theta_table,
    const int n_particles,
    const int n_theta,
    const int FLOC_INTERNAL_MAX_SCATTERS,
    float* heatmap_flat,
    float* exit_dir_out,
    float* exit_x_out,
    float* exit_y_out,
    ...
    unsigned int seed2)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        // Extra propagation distance used only for finite-size floc encounters.
        // A floc is no longer purely a zero-size point event: after the
        // unresolved internal direction changes, the ray is transported by an
        // approximate residual chord length before re-entering the bulk medium.
        float extra_floc_travel_dist = 0.0f;
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            if (is_floc_event) {
                // Internal aggregate memory-loss branch.
                //
                // A floc encounter is treated as an unresolved porous aggregate
                // interaction rather than a single angular draw. The expected
                // number of internal direction-randomising interactions comes
                // from floc_internal_tau_table[pidx], derived on the host from
                // the fractal floc diameter multiplier.
                //
                // Each internal interaction samples the aggregate HG law FLOC_G.
                // Multiple interactions progressively reduce directional memory
                // and allow large/open aggregates to behave differently from
                // small ones without adding per-band fitted knobs.
                float tau_internal = (float)floc_internal_tau_table[pidx];
                tau_internal = fmaxf(1.0f, tau_internal);

                int n_internal = (int)floorf(tau_internal);
                float tau_frac = tau_internal - (float)n_internal;

                if (rnd_uniform(&stateJITTER) < tau_frac) {
                    n_internal += 1;
                }
                ...
                if (n_internal > FLOC_INTERNAL_MAX_SCATTERS) {
                    n_internal = FLOC_INTERNAL_MAX_SCATTERS;
                }

                for (int si = 0; si < n_internal; ++si) {
                    float theta_i =
                    ...
                    new_angle += sign_i * theta_i;
                }

                new_angle += roughness_jitter;

                // Finite floc object approximation.
                // The scattering event is assumed to occur inside a porous
                // aggregate rather than at a mathematical point.  After the
                // unresolved internal interactions, propagate the ray by an
                // approximate residual chord length through the floc.
                //
                // This uses only the selected effective floc diameter already
                // present in the model.  No detector-tied width, no retro
                // probability, and no per-band fitted factor is introduced.
                float floc_diameter_this = (float)particle_diameter_table[pidx];
                float impact_u = rnd_uniform(&stateJITTER);
                impact_u = fminf(1.0f, fmaxf(0.0f, impact_u));

                // Random chord through a circular/spherical aggregate projected
                // into the 2-D transport plane: L = D * sqrt(1 - b^2).
                // The residual path after internal scattering is approximated
                // as half that chord.
                float chord_factor = sqrtf(fmaxf(0.0f, 1.0f - impact_u * impact_u));
                extra_floc_travel_dist = 0.5f * floc_diameter_this * chord_factor;
            }
            else {
        ...
        float travel_dist = STEP_SIZE + extra_floc_travel_dist;
...
def trace_rays_gpu(angles_init_np, STEP_SIZE, scatter_prob, primary_roughness_std_rad, floc_roughness_std_rad,
                   ...
                   particle_is_floc_table_np, floc_internal_tau_table_np,
    ...
    floc_internal_tau_dev = cp.asarray(floc_internal_tau_table_np, dtype=cp.float64)
...
for wl_idx, wl in enumerate(wavelengths):
    ...
    heatmap = trace_rays_gpu(angles_init, np.float32(STEP_SIZE),
                             ...
                             particle_is_floc_table_np, floc_internal_tau_by_bin,
                             angle_cdf_table_np, theta_table_np,
                             hdf5_file=hdf5_file)
    t1 = time.time()
...
if np.any(particle_is_floc):
    floc_event_df = pd.DataFrame({
        ...
        "event_probability": particle_event_weights,
        "g_by_bin": g_by_bin,
        "floc_internal_tau_by_bin": floc_internal_tau_by_bin,
