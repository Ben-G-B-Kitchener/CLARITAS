...
mie_cache_version = "claritas_5p2_structure_factor_audit_v1"
...
def build_floc_phase_from_split_monomer_and_structure(
    ...
    """
    Build the CLARITAS_50 floc phase law from separated monomer components.
    """
    P = _normalise_profile_for_cdf(primary_form_factor)
...
def trace_phase_array(bin_idx, wavelength_m):
    ...
    I, P_diff, P_internal = build_floc_phase_from_split_monomer_and_structure(
        P, S, rep_diameter, wavelength_m, theta_rad
    )
    pd.DataFrame({
        "theta_deg": theta_deg,
...
if os.path.exists(mie_cache_file):
    ...
    for wl in wavelengths:
        ...
        # Physical 3D angular probability measure.
        #
        # I(theta) is an angular intensity / phase function.
        # The probability of scattering into an angular shell is:
        #
        #     dP ∝ I(theta) * sin(theta) dtheta
        #
        # Without this sin(theta) factor, the zero-degree forward bin can
        # dominate the CDF unrealistically.
        angular_measure = np.sin(theta_rad).astype(np.float64)

        # Numerical guard: keep exactly 0 at theta=0, but avoid accidental
        # negative/NaN values elsewhere.
        ...
        for bin_idx, (radius, weight, refr_index, is_floc) in enumerate(zip(
            ...
            # Keep this as intensity, not probability-weighted PDF, so the
            # diagnostic angular_scattering_profiles.csv still shows the raw
            # angular phase/intensity shape.
            psd_weighted_profile += weight * I

            # Physical 3D scattering-angle CDF.
            pdf_theta = I * angular_measure
            pdf_theta[~np.isfinite(pdf_theta)] = 0.0
            pdf_theta = np.maximum(pdf_theta, 0.0)

            # Safety fallback: if a pathological phase function has zero
            # probability after sin(theta) weighting, use the raw I CDF.
            ...
            # Ensure exact final value for binary-search sampling.
            angle_cdf[-1] = 1.0
...
plt.title("Angular scattering profiles - primary Mie + floc split monomer phase with S(q)")
