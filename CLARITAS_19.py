...
# Process-based ray tracer with collision-fraction pooled-mass floc PSD model, explicit mu_s step factor, and diffuse floc backscatter.
...
STEP_SIZE_MU_S_0P1_REFERENCE = 0.1 / mu_s if mu_s > 0.0 else np.inf
STEP_SIZE_MU_S_0P1_REFERENCE_1P0_REFERENCE = 1.0 / mu_s if mu_s > 0.0 else np.inf
STEP_SIZE_MU_S_0P1_REFERENCE_PRIME_REFERENCE = 1.0 / mu_s_prime if mu_s_prime > 0.0 else np.inf
#STEP_SIZE = average_particle_separation_m * STEP_SIZE_MOD
...
print(f"STEP_SIZE_MU_S_0P1_REFERENCE_reference: {STEP_SIZE_MU_S_0P1_REFERENCE:.3e}")
    ...
    unsigned int seed2)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            float theta_3d_projected =
                atan2f(
                    sinf(theta_3d) * cosf(phi),
                    cosf(theta_3d)
                );

            float theta_2d_direct =
                sign2 * theta_3d;

//            const float PROJECTION_BLEND = 1.0f;

            float theta_projected = theta_2d_direct;
 //               theta_3d_projected +
 //               PROJECTION_BLEND *
 //               (theta_2d_direct - theta_3d_projected);
