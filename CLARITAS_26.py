...
# Process-based ray tracer with collision-fraction pooled-mass floc PSD model, explicit mu_s step factor, floc phase-regime mixture, concentration-dependent floc properties, detector-geometry diagnostics, dynamic floc back-fraction, and peaked retro-backscatter lobe.
...
#
# This version adds a physically interpretable concentration effect:
#
#   concentration -> eligible primary number density -> eligible spacing
#                 -> floc mass fraction
#
# The key control is:
#
...
#
# interpreted as an effective mixing/shear/settling interaction length available
# for eligible primary particles to collide and aggregate.
#
# The eligible mass fraction pooled into flocs is:
#
#   floc_fraction = L_collision / (L_collision + s_eligible)
#
# where:
#
#   s_eligible = n_eligible^(-1/3)
#
# This avoids concentration exponents and keeps FLOC_REFLECT_PROB constant.
FLOC_ENABLED = True

# Primary bins at or below the final band edge are eligible for pooling into
# aggregate flocs. Anything above the final band remains an individual particle/bin.
...
# Effective aggregate floc diameter/density are concentration-state dependent.
#
# Low-concentration flocs are treated as smaller and denser.
# High-concentration flocs are treated as larger and looser/more porous.
#
# The active arrays FLOC_POOL_EFFECTIVE_DIAMETER_M and
# FLOC_POOL_EFFECTIVE_DENSITY_KG_PER_M3 are calculated below from the
# concentration-derived floc state parameter.
FLOC_POOL_EFFECTIVE_DIAMETER_LOW_M = np.array([
    20.0e-6,
    25.0e-6,
    30.0e-6,
    35.0e-6,
...
FLOC_POOL_EFFECTIVE_DIAMETER_HIGH_M = np.array([
    40.0e-6,
    50.0e-6,
    60.0e-6,
    70.0e-6,
...
FLOC_POOL_EFFECTIVE_DENSITY_LOW_KG_PER_M3 = np.array([
    1100.0,
    1000.0,
    900.0,
    800.0,
...
FLOC_POOL_EFFECTIVE_DENSITY_HIGH_KG_PER_M3 = np.array([
    900.0,
    800.0,
    700.0,
    600.0,
...
# Active arrays are assigned after floc_encounter_probability is calculated.
FLOC_POOL_EFFECTIVE_DIAMETER_M = FLOC_POOL_EFFECTIVE_DIAMETER_HIGH_M.copy()
FLOC_POOL_EFFECTIVE_DENSITY_KG_PER_M3 = FLOC_POOL_EFFECTIVE_DENSITY_HIGH_KG_PER_M3.copy()

# Physical aggregation opportunity length.
# Larger values transfer more eligible fine-particle mass into pooled flocs.
# Smaller values leave more eligible mass as residual primary particles.
FLOC_COLLISION_LENGTH_M = 400.0e-6
...
#
# Separate primary/floc values let mineral grains and porous aggregates have
# different angular broadening without changing the Mie event-rate calculation.
...
# Primary particles use the Mie angular CDF directly.
#
# Flocs are porous aggregate scatterers, so each floc interaction is sampled from
# a phase-regime mixture rather than treated as a specular reflection.
#
#   forward branch: Mie-sampled forward/transmitted deflection
#   side branch:    broad side escape
#   back branch:    diffuse backward escape centred around 180 degrees
#
# Fractions must sum to 1.0.
FLOC_PHASE_FORWARD_FRACTION = 0.6
FLOC_PHASE_SIDE_FRACTION = 0.1
FLOC_PHASE_BACK_FRACTION = 0.3

if not np.isclose(
    FLOC_PHASE_FORWARD_FRACTION +
    FLOC_PHASE_SIDE_FRACTION +
    FLOC_PHASE_BACK_FRACTION,
    1.0
):
    raise ValueError(
        "FLOC_PHASE_FORWARD_FRACTION + FLOC_PHASE_SIDE_FRACTION + "
        "FLOC_PHASE_BACK_FRACTION must sum to 1.0"
    )

# Scatter-count-dependent floc phase shift.
#
# The baseline CLARITAS_22 phase fractions are preserved at low scatter count.
# As scatter count increases, a bounded amount of the FORWARD branch is shifted
# into the BACK branch. The SIDE branch remains fixed.
#
# This represents loss of forward directional memory after multiple scattering,
# without changing the underlying floc population, event weighting, or side scatter.
FLOC_BACK_FRACTION_SCATTER_GAIN = 0.20
FLOC_BACK_FRACTION_SCATTER_COUNT = 10.0
FLOC_BACK_FRACTION_MAX = 0.70
FLOC_FORWARD_FRACTION_MIN = 0.05

FLOC_SIDE_SCATTER_MIN_DEG = 60.0
FLOC_SIDE_SCATTER_MAX_DEG = 120.0
FLOC_SIDE_SCATTER_MIN_RAD = np.deg2rad(FLOC_SIDE_SCATTER_MIN_DEG)
FLOC_SIDE_SCATTER_MAX_RAD = np.deg2rad(FLOC_SIDE_SCATTER_MAX_DEG)

# Base floc backscatter lobe width.
# This is the low-scatter-count / near-ballistic value that gave the good
# 0.5 g/L fit in CLARITAS_22.
FLOC_BACKSCATTER_STD_BASE_DEG = 5.0

# Maximum lobe width after multiple scattering has reduced directional memory.
# This does not change the floc backscatter probability; it only broadens the
# angular escape distribution for rays that have already undergone many events.
FLOC_BACKSCATTER_STD_MAX_DEG = 15.0

# Scatter-count scale for the smooth broadening:
#
#   std = base + (max - base) * (1 - exp(-scatter_count / scale))
#
# Larger values preserve the CLARITAS_22 low-concentration behaviour more strongly.
FLOC_BACKSCATTER_BROADENING_SCATTER_COUNT = 30.0

# Shape parameter for the peaked retro-backscatter lobe.
#
# The lobe half-width still comes from the dynamic scatter-count-dependent
# backscatter_std_this calculation, but the angular offset is sampled as:
#
#   offset = std * u**power
#
# with random sign. Higher power puts more events close to exact 180-degree
# retro-backscatter while retaining a finite tail.
FLOC_BACKSCATTER_LOBE_POWER = 4.0

FLOC_BACKSCATTER_STD_DEG = FLOC_BACKSCATTER_STD_BASE_DEG
FLOC_BACKSCATTER_STD_RAD = np.deg2rad(FLOC_BACKSCATTER_STD_DEG)
FLOC_BACKSCATTER_STD_MAX_RAD = np.deg2rad(FLOC_BACKSCATTER_STD_MAX_DEG)
...
# Floc angular physics is controlled by the phase-regime fractions above.
...
if FLOC_ENABLED:
    ...
    # Concentration-dependent floc property state.
    # 0 -> smaller/denser low-concentration flocs.
    # 1 -> larger/looser high-concentration flocs.
    floc_property_state = floc_encounter_probability

    FLOC_POOL_EFFECTIVE_DIAMETER_M = (
        (1.0 - floc_property_state) *
        FLOC_POOL_EFFECTIVE_DIAMETER_LOW_M +
        floc_property_state *
        FLOC_POOL_EFFECTIVE_DIAMETER_HIGH_M
    )

    FLOC_POOL_EFFECTIVE_DENSITY_KG_PER_M3 = (
        (1.0 - floc_property_state) *
        FLOC_POOL_EFFECTIVE_DENSITY_LOW_KG_PER_M3 +
        floc_property_state *
        FLOC_POOL_EFFECTIVE_DENSITY_HIGH_KG_PER_M3
    )
    ...
    FLOC_POOL_EFFECTIVE_DIAMETER_M = FLOC_POOL_EFFECTIVE_DIAMETER_LOW_M.copy()
    FLOC_POOL_EFFECTIVE_DENSITY_KG_PER_M3 = FLOC_POOL_EFFECTIVE_DENSITY_LOW_KG_PER_M3.copy()
...
# ------------------------------------------------------------------
# Mie scattering by large particles/flocs is strongly forward-peaked.
# In transport theory, the relevant direction-randomising coefficient is:
#
#   mu_s_prime = mu_s * (1 - g)
#
# where g = <cos(theta)> is the asymmetry parameter.
#
# For a polydisperse PSD, use the interaction-weighted mean g:
#
#   g_eff = sum_i(mu_s_i * g_i) / sum_i(mu_s_i)
#
# Then:
#
#   mu_s_prime = sum_i(mu_s_i * (1 - g_i))
#
# This preserves the process-based event weighting while allowing the transport
# step length to reflect the fact that small-angle scattering does not fully
# randomise direction.
g_by_bin = []
for r, m_rel in zip(particle_radius_m, relative_refractive_index_by_bin):
    x_mie = 2.0 * np.pi * n_medium * r / scatter_probability_wavelength
...
print(f"FLOC_PHASE_FORWARD_FRACTION: {FLOC_PHASE_FORWARD_FRACTION:.3f}")
print(f"FLOC_PHASE_SIDE_FRACTION: {FLOC_PHASE_SIDE_FRACTION:.3f}")
print(f"FLOC_PHASE_BACK_FRACTION: {FLOC_PHASE_BACK_FRACTION:.3f}")
print(f"FLOC_BACK_FRACTION_SCATTER_GAIN: {FLOC_BACK_FRACTION_SCATTER_GAIN:.3f}")
print(f"FLOC_BACK_FRACTION_SCATTER_COUNT: {FLOC_BACK_FRACTION_SCATTER_COUNT:.3f}")
print(f"FLOC_BACK_FRACTION_MAX: {FLOC_BACK_FRACTION_MAX:.3f}")
print(f"FLOC_FORWARD_FRACTION_MIN: {FLOC_FORWARD_FRACTION_MIN:.3f}")
print(f"FLOC_SIDE_SCATTER_RANGE_DEG: {FLOC_SIDE_SCATTER_MIN_DEG:.3f} - {FLOC_SIDE_SCATTER_MAX_DEG:.3f}")
print(f"FLOC_BACKSCATTER_STD_BASE_DEG: {FLOC_BACKSCATTER_STD_BASE_DEG:.3f}")
print(f"FLOC_BACKSCATTER_STD_MAX_DEG: {FLOC_BACKSCATTER_STD_MAX_DEG:.3f}")
print(f"FLOC_BACKSCATTER_BROADENING_SCATTER_COUNT: {FLOC_BACKSCATTER_BROADENING_SCATTER_COUNT:.3f}")
print(f"FLOC_BACKSCATTER_LOBE_POWER: {FLOC_BACKSCATTER_LOBE_POWER:.3f}")
...
mie_cache_version = "per_particle_intensity_no_sin_theta_collision_fraction_pooled_floc_9bin_v1"
...
__global__ void trace_kernel(
    ...
    const float FLOC_PHASE_FORWARD_FRACTION,
    const float FLOC_PHASE_SIDE_FRACTION,
    const float FLOC_PHASE_BACK_FRACTION,
    const float FLOC_BACK_FRACTION_SCATTER_GAIN,
    const float FLOC_BACK_FRACTION_SCATTER_COUNT,
    const float FLOC_BACK_FRACTION_MAX,
    const float FLOC_FORWARD_FRACTION_MIN,
    const float FLOC_SIDE_SCATTER_MIN_RAD,
    const float FLOC_SIDE_SCATTER_MAX_RAD,
    const float FLOC_BACKSCATTER_STD_RAD,
    const float FLOC_BACKSCATTER_STD_MAX_RAD,
    const float FLOC_BACKSCATTER_BROADENING_SCATTER_COUNT,
    const float FLOC_BACKSCATTER_LOBE_POWER,
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            if (is_floc_event) {
                ...
                // Scatter-count-dependent floc phase fractions.
                // SIDE is fixed. A bounded amount of FORWARD probability shifts
                // into BACK as accumulated scatter count increases.
                float dynamic_weight = 0.0f;

                if (FLOC_BACK_FRACTION_SCATTER_COUNT > 0.0f) {
                    dynamic_weight =
                        1.0f -
                        expf(
                            -((float)scatter_count) /
                            FLOC_BACK_FRACTION_SCATTER_COUNT
                        );
                }

                float back_fraction_this =
                    FLOC_PHASE_BACK_FRACTION +
                    FLOC_BACK_FRACTION_SCATTER_GAIN *
                    dynamic_weight;

                if (back_fraction_this > FLOC_BACK_FRACTION_MAX) {
                    back_fraction_this = FLOC_BACK_FRACTION_MAX;
                }

                float side_fraction_this =
                    FLOC_PHASE_SIDE_FRACTION;

                float forward_fraction_this =
                    1.0f -
                    side_fraction_this -
                    back_fraction_this;

                if (forward_fraction_this < FLOC_FORWARD_FRACTION_MIN) {
                    forward_fraction_this = FLOC_FORWARD_FRACTION_MIN;
                    back_fraction_this =
                        1.0f -
                        side_fraction_this -
                        forward_fraction_this;
                }
                ...
                if (phase_sum > 0.0f) {
                    forward_fraction_this /= phase_sum;
                    side_fraction_this /= phase_sum;
                    back_fraction_this /= phase_sum;
                }
                else {
                    forward_fraction_this = 1.0f;
                    side_fraction_this = 0.0f;
                    back_fraction_this = 0.0f;
                }

                float u_phase =
                    rnd_uniform(&stateJITTER);

                if (u_phase < forward_fraction_this) {
                    // Forward/transmitted floc branch:
                    // keep the Mie-sampled deflection.
                    new_angle =
                        ray_angle +
                        theta_projected +
                        roughness_jitter;
                    ...
                    // Diffuse side-escape floc branch.
                    float u_side =
                        rnd_uniform(&stateJITTER);

                    float theta_side =
                        FLOC_SIDE_SCATTER_MIN_RAD +
                        u_side *
                        (FLOC_SIDE_SCATTER_MAX_RAD - FLOC_SIDE_SCATTER_MIN_RAD);
                    ...
                    // Diffuse backward floc branch:
                    // centred around 180 degrees from the incoming ray.
                    //
                    // As scatter_count increases, the ray has lost more memory of
                    // its original incident direction. Broaden the floc backscatter
                    // lobe smoothly with accumulated scatter count, while preserving
                    // the original narrow CLARITAS_22 lobe for low-scatter-count rays.
                    float backscatter_std_this =
                        FLOC_BACKSCATTER_STD_RAD;

                    if (FLOC_BACKSCATTER_BROADENING_SCATTER_COUNT > 0.0f) {
                        float broadening_weight =
                            1.0f -
                            expf(
                                -((float)scatter_count) /
                                FLOC_BACKSCATTER_BROADENING_SCATTER_COUNT
                            );

                        backscatter_std_this =
                            FLOC_BACKSCATTER_STD_RAD +
                            (
                                FLOC_BACKSCATTER_STD_MAX_RAD -
                                FLOC_BACKSCATTER_STD_RAD
                            ) *
                            broadening_weight;
                    }
                ...
                // Primary particle branch: Mie transport, with optional legacy
                // primary reflection retained for controlled tests.
...
def trace_rays_gpu(angles_init_np, STEP_SIZE, scatter_prob, primary_roughness_std_rad, floc_roughness_std_rad,
                   ...
                   floc_back_fraction_scatter_gain,
                   floc_back_fraction_scatter_count,
                   floc_back_fraction_max,
                   floc_forward_fraction_min,
                   floc_side_scatter_min_rad,
                   floc_side_scatter_max_rad,
                   floc_backscatter_std_rad,
                   floc_backscatter_std_max_rad,
                   floc_backscatter_broadening_scatter_count,
                   floc_backscatter_lobe_power,
                ...
                try:
                    trace_kernel((blocks,), (threads_per_block,),
                                 (np.float32(MAX_ITERATIONS),
                                  ...
                                  np.float32(floc_phase_forward_fraction),
                                  np.float32(floc_phase_side_fraction),
                                  np.float32(floc_phase_back_fraction),
                                  np.float32(floc_back_fraction_scatter_gain),
                                  np.float32(floc_back_fraction_scatter_count),
                                  np.float32(floc_back_fraction_max),
                                  np.float32(floc_forward_fraction_min),
                                  np.float32(floc_side_scatter_min_rad),
                                  np.float32(floc_side_scatter_max_rad),
                                  np.float32(floc_backscatter_std_rad),
                                  np.float32(floc_backscatter_std_max_rad),
                                  np.float32(floc_backscatter_broadening_scatter_count),
                                  np.float32(floc_backscatter_lobe_power),
