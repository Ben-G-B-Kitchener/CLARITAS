...
#   - The primary-size band table defines which bins become flocs.
#   - Bins above the final floc band remain individual particles.
#   - Floc bins receive a banded lower effective density.
#   - Effective floc diameter is then calculated by mass conservation.
...
# ============================ MASS-CONSERVING FLOC MODEL ============================
# No independent target floc diameters are specified here.
#
# Instead, the primary-size band table defines which bins flocculate and what
# effective density each floc band has. The effective floc diameter is calculated
# by conserving the representative particle/bin mass:
#
#   rho_solid * D_primary^3 = rho_floc * D_floc^3
#
# therefore:
#
#   D_floc = D_primary * (rho_solid / rho_floc)^(1/3)
#
# This prevents unphysical combinations such as very large floc diameters with
# densities that imply far less solid volume than the original particle mass.
#
# Anything above the final primary band remains an individual particle.
FLOC_PRIMARY_BAND_MAX_DIAMETER_M = np.array([
    0.5e-6,
    1.0e-6,
    2.0e-6,
    5.0e-6,
...
# Banded floc effective density values assigned to the primary-size bands above.
# Edit these directly if settling/microscopy measurements suggest different values.
...
# Per-bin assigned floc density. Non-floc bins remain at original density.
...
# For diagnostics: density assigned before final clipping.
...
# For diagnostics: mass-conserving target diameter before multiplier clipping.
target_floc_diameter_by_bin = primary_particle_diameter_m.copy()

if FLOC_ENABLED:
    particle_is_floc = (
        primary_particle_diameter_m <= FLOC_PRIMARY_BAND_MAX_DIAMETER_M[-1]
    )

    # Assign one floc density per primary-size band.
    for band_max_diameter, band_density in zip(
        FLOC_PRIMARY_BAND_MAX_DIAMETER_M,
    ...
    # receive the final density value.
    ...
    # Mass-conserving effective floc diameter:
    # rho_solid * D_primary^3 = rho_floc * D_floc^3
...
print("FLOC_PRIMARY_BAND_MAX_DIAMETER_um:", np.round(FLOC_PRIMARY_BAND_MAX_DIAMETER_M * 1e6, 3).tolist())
print(f"effective_floc_cutoff_from_final_band_um: {FLOC_PRIMARY_BAND_MAX_DIAMETER_M[-1]*1e6:.3f}")
...
mie_cache_version = "per_particle_intensity_no_sin_theta_mass_conserving_floc_model_v1"
...
mie_cache_key = (
    ...
    str(FLOC_ENABLED) +
    str(FLOC_PRIMARY_BAND_MAX_DIAMETER_M.tolist()) +
    ...
    str(target_floc_density_by_bin.tolist()) +
    str(target_floc_diameter_by_bin.tolist()) +
...
df_psd_compare = pd.DataFrame({
    ...
    "mass_conserving_floc_diameter_um": target_floc_diameter_by_bin * 1e6,
