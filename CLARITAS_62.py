...
def floc_synthetic_structure_profile(bin_idx, wavelength_m, mu_values, theta_rad_values):
    """
    CLARITAS_62 fixed floc phase function.

    This is a drop-in replacement for CLARITAS_61/62 that leaves the PSD,
    cross-sections, event weighting and CUDA transport unchanged.

    Difference from CLARITAS_61:
      - the diffraction component is generated at the aggregate/floc diameter,
        not at the representative monomer diameter;
      - the coherent internal component still uses the existing structure
        factor and the representative-primary internal Mie component;
      - the existing CLARITAS_60 internal-transport helper is used with its
        actual four-value return signature.
    """
    n_synth, n_physical, rep_idx = _representative_monomer_count_for_floc_bin(bin_idx)
    ...
    # Aggregate-scale diffraction from the whole floc.  This is deliberately
    # separated from the monomer internal component so the floc phase does not
    # collapse back to representative-primary Mie when S(q) is near unity.
    ...
    # Preserve the raw angular weight of the removed monomer diffraction term.
    # Event rate is still controlled only by sigma_t / extinction weighting.
    ...
    if FLOC_INTERNAL_TRANSPORT_ENABLED:
        transported_internal, tau_s, tau_transport, f_transport = (
            _floc_internal_transport_component(
                bin_idx,
                primary_internal,
                rep_diameter,
        ...
        f_transport = float(np.clip(f_transport, 0.0, 1.0))
        internal_component = (
            (1.0 - f_transport) * coherent_internal +
            f_transport * transported_internal
        )

        # The helper already stores its standard diagnostics.  Add only the
        # aggregate-scale diffraction diagnostics here without replacing them.
        diag = floc_internal_transport_diagnostics.get(int(bin_idx), {})
        diag.update({
            "aggregate_phase_model": "aggregate_scale_diffraction_plus_internal_transport",
            ...
            "transport_fraction": float(f_transport),
        })
