# CLARITAS_102: Polarised vector radiative transfer Monte Carlo
...
#
# Physics (v102 improvements over v101):
...
#   - NEW: Diffraction peak smoothing for irregular particles (θ < 5°)
#   - NEW: R scaling uses transport MFP (accounts for scattering anisotropy)
...
if _CUPY_AVAILABLE:
    ...
    def trace_rays_gpu(angles_init_np, R, ray_off, beam_sigma, collimated,
        ...
        with h5py.File(hdf5_file, "w") as f:
            f.attrs["claritas_version"] = "102_gpu"
...
print("="*70)
print("CLARITAS_102: Polarised Vector Radiative Transfer Monte Carlo (v102: transport τ*, diffraction smoothing)")
...
plt.title("Detector Response — CLARITAS_102 Polarised MC")
...
if len(wavelengths)==1:
    ...
    plt.suptitle(f"Polarisation Diagnostics — CLARITAS_102 — {wl_nm} nm",fontsize=14,fontweight="bold")
    plt.tight_layout()
...
for wl_idx,wl in enumerate(wavelengths):
    ...
    if os.path.exists(hf):
        ...
        plt.title(f"Scatter Count Histogram — CLARITAS_102 — {wl_nm} nm")
