...
def _pair_distance_structure_factor(theta_rad_values, wavelength_m, points):
    ...
    phase function.  In CLARITAS_47 the floc phase function is built as:
    ...
    avoids constructing a huge q-by-pair array.
    """
...
def floc_synthetic_structure_profile(bin_idx, wavelength_m, mu_values, theta_rad_values):
    """
    CLARITAS_48 floc aggregate phase function.

    The floc is treated as a correlated collection of representative primary
    particles.  The angular law is therefore:

        I_floc(theta) = P_primary(theta) * S_aggregate(q)

    P_primary(theta) is the Mie phase function of the representative source
    primary particle for this floc band.  S_aggregate(q) is the Debye
    pair-distance structure factor from the generated aggregate geometry.

    This deliberately does not use HG, retroreflection, detector tuning, or a
    structure-only isotropic phase function.
    """
    ...
    I = primary_form_factor * structure_factor
...
def trace_phase_array(bin_idx, wavelength_m):
    ...
    I = _normalise_profile_for_cdf(P*S)
    pd.DataFrame({
        "theta_deg":theta_deg,
...
if os.path.exists(mie_cache_file):
    ...
    print("Building angular tables: primary Mie + Mie-times-structure floc aggregate diagnostics...")
...
plt.title("Angular scattering profiles - primary Mie + floc P_primary times S(q)")
...
def export_phase_function_audit_for_representative_floc(wl_idx=0, target_floc_diameter_um=120.0):
    ...
    final_intensity = primary_form_factor * structure_factor
    final_intensity = _normalise_profile_for_cdf(final_intensity)

    final_pdf = final_intensity / np.sum(final_intensity)
    cpu_cdf_recomputed = np.cumsum(final_intensity).astype(np.float64)
    cpu_cdf_recomputed /= cpu_cdf_recomputed[-1]
    ...
    phase_audit_df = pd.DataFrame({
        ...
        "final_pdf_sum_normalised": final_pdf,
...
if np.any(particle_is_floc):
    trace_phase_array(np.where(particle_is_floc)[0][0], wavelengths[0])
