#!/usr/bin/env python3
"""CLARITAS effective-PSD inference V3.4 using V24.4 reconstructed-hardware TARDIIS detector statistics.

This is an inverse-model wrapper around the CLARITAS V24.4 3-D geometric / Snell / Fresnel particle physics plus the
physical TARDIIS cylindrical water/acrylic/air cell and reconstructed sensor-ring collimator.
The shared forward implementation is ``claritas_tardiis_core_v24_4.py``.

V3.4 retains the broader suspension-state parameterisation introduced in V2.  Instead of a
...
there is no post-stir settling clock in V3.4:
...
from claritas_tardiis_core_v24_4 import TardiisForwardModel as ClaritasForwardModel, get_material_psd
...
class PSDInferenceV34:
    def __init__(self, material: str, measurements: pd.DataFrame, config: dict,
        ...
        if self.concentrations.size < 2:
            raise ValueError("V3.4 expects at least two measured concentrations")
    ...
    def load_initial_parameters(self, path: str | None) -> np.ndarray:
        ...
        if p.size != self.n_params:
            raise ValueError(f"Initial parameter file has {p.size} parameters; V3.4 expects {self.n_params}")
        print(f"Loaded initial parameters from {path}")
        return p

    def load_v1_warm_start(self, path: str) -> np.ndarray:
        """Approximate V1 effective PSDs with the V3.4 seven-knot representation."""
        df = pd.read_csv(path)
        ...
        print(f"Warm-started V3.4 PSD controls from V1 effective PSDs: {path}")
        return p

    def optimise(self, initial_parameters: str | None = None, skip_local: bool = False,
                 v1_warm_start: str | None = None):
        print(f"\n=== {self.material.upper()} PSD inference V3.4 / V24.4 reconstructed TARDIIS apparatus ===")
        ...
        print("V24.4 TARDIIS apparatus + particle optical physics is fixed. Selected detector scoring is the reconstructed 4-mm through-bore / 8.7-mm counterbore hard geometry with x-reflection symmetry averaging; the legacy +/-6.5-degree scorer is diagnostic only. No KDE/extrapolation. Only suspension-state PSD and local concentration are inferred.\n")
    ...
    def write_outputs(self, opt_result, p_best: np.ndarray, final_score: float,
        ...
        for x in conditions:
            ...
            for jj, (a, mv, mod) in enumerate(zip(res.detector_angles_deg, measured, res.normalized_response)):
                detector_rows.append({
                    ...
                    "modelled_normalized_hardware_symmetry": float(res.normalized_hardware_symmetry_response[jj]),
                    "modelled_normalized_hardware_native": float(res.normalized_hardware_native_response[jj]),
                    "modelled_normalized_hardware_mirror": float(res.normalized_hardware_mirror_response[jj]),
                    "hardware_symmetry_physical_equivalent_score": float(res.hardware_symmetry_scores[jj]),
                    "hardware_native_hits": int(res.hardware_native_hits[jj]),
                    "hardware_mirror_hits": int(res.hardware_mirror_hits[jj]),
                    "modelled_normalized_legacy_6p5_symmetry": float(res.normalized_legacy_symmetry_response[jj]),
                    "legacy_6p5_symmetry_channel_score": float(res.legacy_symmetry_channel_scores[jj]),
        ...
        payload = {
            ...
            "detector_scoring": "V24.4 reconstructed 4-mm through-bore + 8.7-mm counterbore hard geometry with x-reflection symmetry average; legacy +/-6.5-degree output diagnostic only",
        ...
        plt.title(f"{self.material.capitalize()} V3.4 inferred optical PSD modification")
        ...
        plt.title(f"{self.material.capitalize()} source and V3.4 effective PSDs")
        plt.grid(True, which="both", alpha=0.25); plt.legend(); plt.tight_layout()
        ...
        for x in conditions:
            ...
            plt.plot(x["result"].detector_angles_deg, x["result"].normalized_response, "--",
                     label=f"V3.4 fit {c:g} g/L")
        plt.xlabel("Detector angle (deg)"); plt.ylabel("Normalized detector response")
        plt.title(f"{self.material.capitalize()} measured vs V3.4 exact TARDIIS-apparatus inferred-suspension response")
        ...
        plt.title(f"{self.material.capitalize()} V3.4 detector residuals")
...
def main():
    ap = argparse.ArgumentParser(
        description="CLARITAS PSD Inference V3.4: V24.4 reconstructed TARDIIS apparatus + non-monotonic effective PSD/local concentration inference")
    ap.add_argument("--material", choices=["kaolin", "loess", "all"], default="all")
    ap.add_argument("--measurements", default="measured_detector_responses_v3_4.csv")
    ap.add_argument("--config", default="psd_inference_v3_4_config.json")
    ap.add_argument("--output-dir", default="psd_inference_v3_4_results")
    ...
    ap.add_argument("--warm-start-v1", help="V1 effective_psds.csv used to initialise the seven-knot V3.4 PSDs")
    ...
    for mat in materials:
        inf = PSDInferenceV34(mat, df, cfg, root / mat, quick=args.quick)
        if args.baseline_only:
            score = inf.baseline_only()
            print(f"{mat} V3.4 baseline objective: {score:.8g}")
        else:
