#!/usr/bin/env python3
"""CLARITAS effective-PSD inference V3.2 using V24.2 multi-bandwidth zero-bandwidth-extrapolated TARDIIS detector scoring.

This is an inverse-model wrapper around the CLARITAS V24.2 3-D geometric / Snell / Fresnel particle physics plus the
physical TARDIIS cylindrical water/acrylic/air cell and detector collimators.
The shared forward implementation is ``claritas_tardiis_core_v24_2.py``.

V3 retains the broader suspension-state parameterisation introduced in V2.  Instead of a
...
there is no post-stir settling clock in V3:
...
from claritas_tardiis_core_v24_2 import TardiisForwardModel as ClaritasForwardModel, get_material_psd
...
class PSDInferenceV32:
    def __init__(self, material: str, measurements: pd.DataFrame, config: dict,
        ...
        if self.concentrations.size < 2:
            raise ValueError("V3 expects at least two measured concentrations")
    ...
    def load_initial_parameters(self, path: str | None) -> np.ndarray:
        ...
        if p.size != self.n_params:
            raise ValueError(f"Initial parameter file has {p.size} parameters; V3 expects {self.n_params}")
        print(f"Loaded initial parameters from {path}")
        return p

    def load_v1_warm_start(self, path: str) -> np.ndarray:
        """Approximate V1 effective PSDs with the V3 seven-knot representation."""
        df = pd.read_csv(path)
        ...
        print(f"Warm-started V3 PSD controls from V1 effective PSDs: {path}")
        return p

    def optimise(self, initial_parameters: str | None = None, skip_local: bool = False,
                 v1_warm_start: str | None = None):
        print(f"\n=== {self.material.upper()} PSD inference V3.2 / TARDIIS apparatus + multi-bandwidth extrapolated detector scoring ===")
        ...
        print("V24.2 TARDIIS apparatus + particle optical physics is fixed; detector scoring uses fixed multi-bandwidth h->0 extrapolation and is non-fitted. Only suspension-state PSD and local concentration are inferred.\n")
    ...
    def write_outputs(self, opt_result, p_best: np.ndarray, final_score: float,
        ...
        for x in conditions:
            ...
            for jj, (a, mv, mod) in enumerate(zip(res.detector_angles_deg, measured, res.normalized_response)):
                row={
                    ...
                    "modelled_normalized_exact": float(res.normalized_exact_response[jj]),
                    "raw_model_hits": int(res.raw_hits[jj]),
                    "multiband_extrapolated_physical_equivalent_score": float(res.multiband_extrapolated_physical_equivalent_scores[jj]),
                    "multiband_extrapolated_raw_intercept": float(res.multiband_extrapolated_raw_scores[jj]),
                    "multiband_fit_r2": float(res.multiband_fit_r2[jj]),
                    "multiband_relative_spread": float(res.multiband_relative_spread[jj]),
                    "residual": mod - mv,
                }
                for bi,bf in enumerate(res.multiband_bandwidth_factors):
                    lab=("%g"%float(bf)).replace(".","p")
                    row[f"modelled_normalized_bw_{lab}"]=float(res.multiband_normalized_responses[bi,jj])
                    row[f"physical_equivalent_bw_{lab}"]=float(res.multiband_physical_equivalent_scores[bi,jj])
                    row[f"support_rays_bw_{lab}"]=int(res.multiband_support_counts[bi,jj])
            ...
            summary_rows.append({
                ...
                "detector_multiband_equivalent_fraction": res.detector_multiband_equivalent_fraction,
                "detector_scoring_mode": res.detector_scoring_mode,
                "detector_vr_bandwidth_factors": ",".join(f"{q:g}" for q in res.detector_vr_bandwidth_factors),
                "detector_extrapolation_power": res.detector_extrapolation_power,
                "detector_extrapolation_median_r2": float(np.nanmedian(res.multiband_fit_r2)),
                "detector_extrapolation_negative_raw_channels": int(np.count_nonzero(res.multiband_extrapolated_raw_scores < 0)),
                "detector_largest_bandwidth_min_support": int(np.min(res.multiband_support_counts[-1])),
                "detector_largest_bandwidth_median_support": float(np.median(res.multiband_support_counts[-1])),
            })
        ...
        payload = {
            ...
            "detector_scoring": "V24.2 multi-bandwidth 4-D product-Epanechnikov physical-equivalent scores extrapolated in h^2 to h->0; exact physical hits retained",
        ...
        plt.title(f"{self.material.capitalize()} V3.2 inferred optical PSD modification")
        ...
        plt.title(f"{self.material.capitalize()} source and V3.2 effective PSDs")
        plt.grid(True, which="both", alpha=0.25); plt.legend(); plt.tight_layout()
        ...
        for x in conditions:
            ...
            plt.plot(x["result"].detector_angles_deg, x["result"].normalized_response, "--",
                     label=f"V3.2 fit {c:g} g/L")
        plt.xlabel("Detector angle (deg)"); plt.ylabel("Normalized detector response")
        plt.title(f"{self.material.capitalize()} measured vs V3.2 TARDIIS-apparatus inferred-suspension response")
        ...
        plt.title(f"{self.material.capitalize()} V3.2 detector residuals")
...
def main():
    ap = argparse.ArgumentParser(
        description="CLARITAS PSD Inference V3.2: V24 TARDIIS apparatus + non-monotonic effective PSD/local concentration inference")
    ap.add_argument("--material", choices=["kaolin", "loess", "all"], default="all")
    ap.add_argument("--measurements", default="measured_detector_responses_v3_2.csv")
    ap.add_argument("--config", default="psd_inference_v3_2_config.json")
    ap.add_argument("--output-dir", default="psd_inference_v3_2_results")
    ap.add_argument("--quick", action="store_true", help="100k-ray / capped-evaluation exploratory fit")
    ...
    ap.add_argument("--warm-start-v1", help="V1 effective_psds.csv used to initialise the seven-knot V3 PSDs")
    ...
    for mat in materials:
        inf = PSDInferenceV32(mat, df, cfg, root / mat, quick=args.quick)
        if args.baseline_only:
            score = inf.baseline_only()
            print(f"{mat} V3.2 baseline objective: {score:.8g}")
        else:
