#!/usr/bin/env python3
"""Shared CLARITAS V24 / TARDIIS apparatus forward model.
...
def detect_physical_apertures(x,y,z,vx,vy,vz,valid,inner_plane_radius_m,outer_plane_radius_m,aperture_radius_m):
    """Assign each air-exit ray to at most one of the 18 physical TARDIIS collimators."""
    p=np.column_stack([x,y,z]).astype(float);v=np.column_stack([vx,vy,vz]).astype(float)
    ...
    for j,deg in enumerate(DETECTOR_ANGLES_DEG):
        ...
        q1=p+t1[:,None]*v-inner_plane_radius_m*u[None,:]
        q2=p+t2[:,None]*v-outer_plane_radius_m*u[None,:]
        rr=aperture_radius_m*aperture_radius_m
        good &= (np.einsum('ij,ij->i',q1,q1)<=rr) & (np.einsum('ij,ij->i',q2,q2)<=rr)
        upd=good & (t1<best_t);best_t[upd]=t1[upd];best[upd]=j
...
class ForwardResult:
    ...
    detector_detection_fraction: float
...
class TardiisForwardModel:
    ...
    def simulate(self,material,concentration_g_per_L,weights=None,n_rays=100000,seed=12345,weight_mode='mass_fraction',collect_rays=False,heatmap_size=0):
        ...
        hits,det_id=detect_physical_apertures(arrays['air_x'],arrays['air_y'],arrays['air_z'],arrays['air_vx'],arrays['air_vy'],arrays['air_vz'],valid_air,self.coll_in,self.coll_out,self.aperture_radius)
        total=int(hits.sum());norm=hits.astype(float)/total if total>0 else np.zeros_like(hits,dtype=float)
        entered=ints['entered_water']>0;den=max(int(entered.sum()),1);ic=ints['interaction_count'][entered]
        ray_data=None
        if collect_rays:
            ray_data={**arrays,**ints,'detector_index':det_id,'detector_deg':np.where(det_id>=0,DETECTOR_ANGLES_DEG[np.clip(det_id,0,len(DETECTOR_ANGLES_DEG)-1)],np.nan)}
        hmxy=cp.asnumpy(hxy).reshape(vis,vis) if vis>0 else None;hmzy=cp.asnumpy(hzy).reshape(vis,vis) if vis>0 else None
        return ForwardResult(
            detector_angles_deg=DETECTOR_ANGLES_DEG.copy(),raw_hits=hits,normalized_response=norm,
            ...
            air_exit_fraction=float(np.mean(valid_air)),axial_loss_fraction=float(np.mean(ints['status']==2)),detector_detection_fraction=float(total/N),
