...
# Process-based ray tracer with collision-fraction pooled-mass floc PSD model, explicit mu_s step factor, floc phase-regime mixture, concentration-dependent floc properties, detector-geometry diagnostics, and scatter-count-dependent floc backscatter broadening.
...
FLOC_BACKSCATTER_STD_MAX_DEG = 45.0
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            if (is_floc_event) {
                ...
                if (u_phase < FLOC_PHASE_FORWARD_FRACTION) {
