...
# Process-based ray tracer with collision-fraction pooled-mass floc PSD model, explicit mu_s step factor, diffuse floc backscatter, and pair-encounter floc formation.
...
# Pair/cooperative encounter order for floc formation.
# 1.0 gives the previous linear occupancy model.
# 2.0 requires pairwise encounter/retention and suppresses low-concentration floccing.
FLOC_PAIR_ENCOUNTER_ORDER = 2.0
...
# Diffuse floc backscatter lobe.
# When a floc reflection/backscatter event occurs, do not use a brittle
# specular 180-degree flip. Instead redirect into a backward lobe centred
# around pi radians relative to the incoming ray.
FLOC_BACKSCATTER_STD_DEG = 60.0
...
if FLOC_ENABLED:
    ...
    if eligible_primary_number_density_per_m3 > 0.0:
        ...
        # Previous model was linear occupancy:
        #   L / (L + spacing)
        #
        # This version uses pair/cooperative encounter probability:
        #   [L / (L + spacing)] ** FLOC_PAIR_ENCOUNTER_ORDER
        #
        # With FLOC_PAIR_ENCOUNTER_ORDER = 2.0, low-concentration floc formation
        # is suppressed more strongly while keeping FLOC_REFLECT_PROB constant.
...
print(f"FLOC_PAIR_ENCOUNTER_ORDER: {FLOC_PAIR_ENCOUNTER_ORDER:.3f}")
    ...
    unsigned int seed2)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            bool stochastic_reflect =
                (rnd_uniform(&stateREFL) < reflect_prob_this) &&
            ...
            if (stochastic_reflect || path_reflect) {

                if (is_floc_event) {
                    // Diffuse backward floc lobe:
                    // centre the scattered direction around 180 degrees from
                    // the incoming ray, with Gaussian angular spread.
                    new_angle =
                        ray_angle +
                        3.1415927f +
                    ...
                    // Primary particles retain the previous specular-like flip.
