...
#   - floc angular scattering is sampled from HG(FLOC_G)
#
...
# Floc aggregate phase anisotropy used by the HG angular sampler.
#
...
# ============================ COMPLEX REFRACTIVE INDEX / ALBEDO MODEL ============================
# Complex refractive index convention used here:
#   m = n - i*k
# where k >= 0 represents absorption in the Mie calculation.
#
# Primary particles:
#   - use complex Mie efficiencies
#   - event cross-section = sigma_ext = sigma_scat + sigma_abs
#   - event albedo = sigma_scat / sigma_ext
#   - if an event is absorption, the ray is terminated rather than redirected
#
# Flocs:
#   - still use the aggregate HG angular law for the scattering direction
#   - use a density-scaled effective imaginary index to estimate absorption
#   - larger and/or denser flocs therefore have lower albedo by optical-depth
#   - event cross-section = sigma_scat + sigma_abs
#
# This keeps the previous best aggregate angular kernel but enforces the
# albedo principle for both primary particles and flocs.

n_particle_real = 1.59
n_particle_imag = 1.0e-4
n_particle = complex(n_particle_real, -n_particle_imag)
n_medium = 1.33
n_external = 1.0  # refractive index outside circular sample boundary, e.g. air

# Effective floc absorption parameters.
# The floc imaginary index is density-scaled from the solid-particle k:
#   k_floc = FLOC_IMAG_SCALE * k_primary * rho_floc / rho_solid
# Absorption cross-section uses a projected-area survival model:
#   sigma_abs_floc = Q_abs * pi*r^2 * (1 - exp(-tau_abs))
#   tau_abs = 4*pi*k_floc*d_floc/lambda
FLOC_IMAG_SCALE = 1.0
FLOC_ABSORPTION_EFFICIENCY = 1.0

# ============================ PRIMARY-PARTICLE REFRACTIVE INDEX ============================
# Primary particles use complex Mie scattering and therefore require a complex refractive index.
# Flocs are NOT treated as Mie spheres; they use an aggregate angular law plus albedo.

particle_refractive_index_by_bin = np.full(
...
if np.any(particle_is_floc):
    particle_refractive_index_by_bin[particle_is_floc] = np.nan + 0.0j
...
# Scattering/extinction cross-section, albedo, and anisotropy per effective bin.
#
# Primary particles:
#   sigma_scat = Qsca * pi*r^2       from complex Mie
#   sigma_abs  = max(Qext-Qsca,0)*pi*r^2
#   sigma_ext  = sigma_scat + sigma_abs
#   albedo     = sigma_scat / sigma_ext
#   g          = Mie asymmetry parameter
#
# Flocs:
#   sigma_scat = FLOC_SCATTER_EFFICIENCY * pi*r^2
#   sigma_abs  = FLOC_ABSORPTION_EFFICIENCY * pi*r^2 * (1-exp(-tau_abs))
#   tau_abs    = 4*pi*k_eff*d/lambda
#   k_eff      = FLOC_IMAG_SCALE * k_primary * rho_floc/rho_solid
#   albedo     = sigma_scat / sigma_ext
#   g          = FLOC_G
#
# Event sampling uses extinction, not scattering only. After an event is
# selected, the CUDA kernel decides scatter vs absorption using the per-bin albedo.
sigma_s = []
sigma_abs = []
sigma_ext = []
g_by_bin = []
particle_albedo_by_bin = []
particle_k_effective_by_bin = []

for r, refr_index, is_floc, density_this, diameter_this in zip(
    particle_radius_m,
    particle_refractive_index_by_bin,
    particle_is_floc,
    particle_density_by_bin_kg_per_m3,
    particle_diameter_m
):
    area = np.pi * r**2

    if is_floc:
        sigma_scat_this = FLOC_SCATTER_EFFICIENCY * area

        density_fraction = np.clip(
            density_this / particle_density_kg_per_m3,
            0.0,
            1.0
        )

        k_eff = FLOC_IMAG_SCALE * n_particle_imag * density_fraction
        tau_abs = (
            4.0 * np.pi * k_eff * diameter_this /
            scatter_probability_wavelength
        )

        sigma_abs_this = (
            FLOC_ABSORPTION_EFFICIENCY *
            area *
            (1.0 - np.exp(-tau_abs))
        )

        sigma_ext_this = sigma_scat_this + sigma_abs_this
        g_this = FLOC_G
        ...
        qext, qsca, qback, g_this = miepython.efficiencies_mx(
            m_rel,
            x_mie
        )
        ...
        sigma_scat_this = qsca * area
        sigma_abs_this = qabs * area
        sigma_ext_this = max(qext * area, sigma_scat_this + sigma_abs_this)
        k_eff = n_particle_imag

    if sigma_ext_this > 0.0:
        albedo_this = np.clip(sigma_scat_this / sigma_ext_this, 0.0, 1.0)
    else:
        albedo_this = 1.0

    sigma_s.append(sigma_scat_this)
    sigma_abs.append(sigma_abs_this)
    sigma_ext.append(sigma_ext_this)
    g_by_bin.append(g_this)
    particle_albedo_by_bin.append(albedo_this)
    particle_k_effective_by_bin.append(k_eff)

sigma_s = np.asarray(sigma_s, dtype=np.float64)
sigma_abs = np.asarray(sigma_abs, dtype=np.float64)
sigma_ext = np.asarray(sigma_ext, dtype=np.float64)
g_by_bin = np.asarray(g_by_bin, dtype=np.float64)
particle_albedo_by_bin = np.asarray(particle_albedo_by_bin, dtype=np.float64)
particle_k_effective_by_bin = np.asarray(particle_k_effective_by_bin, dtype=np.float64)

# Scattering, absorption, and extinction coefficients.
mu_scat_by_bin = particle_number_density_by_bin * sigma_s
mu_abs_by_bin = particle_number_density_by_bin * sigma_abs
mu_ext_by_bin = particle_number_density_by_bin * sigma_ext

mu_scat = np.sum(mu_scat_by_bin)
mu_abs = np.sum(mu_abs_by_bin)
mu_ext = np.sum(mu_ext_by_bin)

# Legacy names retained for downstream diagnostics and step-size logic:
# mu_s_by_bin and mu_s now represent interaction/extinction coefficient.
mu_s_by_bin = mu_ext_by_bin
mu_s = mu_ext

# Reduced scattering coefficient uses scattering-only contribution.
mu_s_prime_by_bin = mu_scat_by_bin * (1.0 - g_by_bin)
mu_s_prime = np.sum(mu_s_prime_by_bin)

if mu_scat > 0.0:
    g_eff = np.sum(mu_scat_by_bin * g_by_bin) / mu_scat
else:
    g_eff = 0.0

bulk_single_scattering_albedo = (
    mu_scat / mu_ext if mu_ext > 0.0 else 1.0
)

# Explicit event step-length control.
# The kernel treats each step as an extinction-event opportunity.
...
# Particle/floc choice during an interaction should be weighted by
# contribution to extinction interactions, not by raw PSD mass fraction.
...
print(
    f"primary_refractive_index: {n_particle_real:.4f} - {n_particle_imag:.2e}j; "
...
print(f"mu_ext: {mu_ext:.3e}")
print(f"mu_scat: {mu_scat:.3e}")
print(f"mu_abs: {mu_abs:.3e}")
print(f"bulk_single_scattering_albedo: {bulk_single_scattering_albedo:.6f}")
...
# ================= ANGULAR PROFILES (host side - primary Mie + floc HG, cached) =================
...
#
# Floc bins:
#   - HG aggregate angular profile using FLOC_G
#   - no floc Mie calculation
#   - no floc refractive index
#
# The CUDA kernel samples angle_cdf_table for primary bins only.
# For flocs it samples HG(FLOC_G) directly inside the kernel.
# Floc CDFs are still included in cdf_profiles as harmless placeholders so the
# table shape remains [n_particles, n_theta] and existing kernel indexing remains simple.

#mie_cache_version = "claritas_3p0_primary_mie_floc_fractal_hg_v1"
mie_cache_version = "claritas_36_complex_albedo_v1"
...
mie_cache_key = (
    ...
    str(n_particle_imag) +
    str(FLOC_IMAG_SCALE) +
    str(FLOC_ABSORPTION_EFFICIENCY) +
    str(n_medium) +
...
def hg_intensity_profile(theta_rad_values, g_value):
    """
    Host-side HG-like angular profile for diagnostics only.

    This mirrors the same anisotropy assumption used by the CUDA floc branch.
    It is not used to sample floc angles in the kernel; the kernel samples HG
    directly from FLOC_G.
    """
    g_value = np.clip(g_value, -0.999, 0.999)

    if abs(g_value) < 1.0e-9:
        return np.ones_like(theta_rad_values, dtype=np.float64)
    ...
    return (
        ...
        (
            1.0 +
            g_value * g_value -
            2.0 * g_value * np.cos(theta_rad_values)
        ) ** 1.5
    ).astype(np.float64)
...
if os.path.exists(mie_cache_file):
    ...
    print("Building angular tables: primary Mie + floc HG diagnostics...")
    ...
    for wl in wavelengths:
        ...
        for radius, weight, refr_index, is_floc in zip(
            particle_radius_m,
            particle_weights,
            particle_refractive_index_by_bin,
            particle_is_floc
            ...
            if is_floc:
                # Flocs are not Mie scatterers in CLARITAS 3.0.
                # This HG profile is for plotting/cache shape only.
                I = hg_intensity_profile(theta_rad, FLOC_G)
...
plt.title("Angular scattering profiles — primary Mie + floc HG")
...
__device__ float sample_henyey_greenstein_theta(
    unsigned int* state,
    float g)
{
...
__global__ void trace_kernel(
    ...
    const double* particle_albedo_table,
    ...
    unsigned int seed2)
{
    ...
    while (x * x + y * y <= R_REAL * R_REAL) {
        ...
        if (rnd_uniform(&state) < scatter_prob) {
            ...
            float albedo_this = (float)particle_albedo_table[pidx];
            ...
            if (is_floc_event) {
                ...
                theta_3d =
                    sample_henyey_greenstein_theta(&stateJITTER, FLOC_G);
                    }
            else {
...
def trace_rays_gpu(angles_init_np, STEP_SIZE, scatter_prob, primary_roughness_std_rad, floc_roughness_std_rad,
                   ...
                   particle_is_floc_table_np, particle_albedo_table_np,
                   angle_cdf_table_np, theta_table_np,
    ...
    particle_albedo_dev = cp.asarray(particle_albedo_table_np, dtype=cp.float64)
...
for wl_idx, wl in enumerate(wavelengths):
    ...
    print("DEBUG FLOC_G passed to GPU:", FLOC_G)
...
df_effective_psd = pd.DataFrame({
    ...
    "single_scattering_albedo": particle_albedo_by_bin,
    "sigma_scat_m2": sigma_s,
    "sigma_abs_m2": sigma_abs,
    "sigma_ext_m2": sigma_ext,
    "mu_scat_by_bin_per_m": mu_scat_by_bin,
    "mu_abs_by_bin_per_m": mu_abs_by_bin,
    "mu_ext_by_bin_per_m": mu_ext_by_bin,
    "effective_k_imag": particle_k_effective_by_bin,
    "g_by_bin": g_by_bin,
...
if np.any(particle_is_floc):
    floc_event_df = pd.DataFrame({
        ...
        "event_probability": particle_event_weights,
        "g_by_bin": g_by_bin,
        "floc_g_by_effective_bin": floc_g_by_effective_bin,
